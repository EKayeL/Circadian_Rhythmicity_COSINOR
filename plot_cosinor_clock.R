...
#' Plot Clock Gene Cosinor Fit Function
#'
...
plot_cosinor_clock <- function(cosMod, original_data, plot_title, data_type,
  ...
  if (is.null(y_limits)) {
    y_limits <- range(plot_data$`Fold_Changes`, plot_data$Fitted, na.rm = TRUE)
  }
  
  # Create the plot
  plot <- ggplot(plot_data, aes(x = `ZT`)) +
    # Scatter plot of original data
    geom_point(aes(y = `Fold_Changes`, color = "Original Data")) +
    geom_smooth(aes(y = `Fitted`, color = "Cosinor"), size = 1,
                linetype = fit_line_type) +  # Fitted line
    xlab("Zeitgeber Time (ZT)") +
    ylab("Number of Fold Changes") +
    ggtitle(plot_title) +
