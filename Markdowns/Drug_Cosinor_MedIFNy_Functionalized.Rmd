---
title: "Drug Group Cosinor Medullary IFNy Functionalized"
...
### Reading, Converting, Analyzing and Plotting Medullary IFNy 
### Administered Rats 
#### Reading in Medullary IFNy Administered Rat Data for 
#### LD and rLEN
```{r}
# Reading Medullary IFNy Administered Rats for LD
readxl::read_excel(path ="../Summary Multiplex Data.xlsx",
                   sheet = "Medulla",
                    range ="N65:R71") -> LD_Circadian_Med_IFNy_Filter

# Reading Medullary IFNy Administered Rats for rLEN
readxl::read_excel(path ="../Summary Multiplex Data.xlsx",
                   sheet = "Medulla",
                    range ="U65:Y71") -> rLEN_Circadian_Med_IFNy_Filter
```


#### Converting Data into Usable Format for Extracting VOIs and Plotting
```{r}
# Data Conversion to get Individual Points for LD Medullary IFNy 
# Rat Model
## X value is Zeitgeber time, Y value is Concentration in mg of Drug/Content

# Loads Initial Dataset for LD Medullary IFNy Rats and 
# Removes Outliers
#outlier_detection(Circadian_Med_IFNy_LD) -> out_res_LDMIFNy
#filter_outliers(Circadian_Med_IFNy_LD, 
#                out_res_LDMIFNy) ->  LD_Circadian_Med_IFNy_Filter

# Performs Data Carpentry on Filtered LD IFNy Dataset and Removes 'NA's,
## filters out ZT of 24
LD_Circadian_Med_IFNy_Filter %>%
  pivot_longer(cols = c(`0`,`6`,`12`, `18`, `24`), names_to = "ZT", 
               values_to = "Concentration(mg)") %>%
  drop_na(`Concentration(mg)`) %>%
  mutate(ZT = as.numeric(ZT)) %>%
  arrange(ZT) -> LD_Circadian_Med_IFNy_Filter
LD_Circadian_Med_IFNy_Filter %>%
  # Filter out ZT of 24
  filter(ZT != 24)  -> LD_Circadian_Med_IFNy_data

# Creating Cosinor Model for LD IFNy Rat Plots
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = LD_Circadian_Med_IFNy_Filter) -> cosMod_LD_MIFNy
# Creating Cosinor Model for LD IFNy Rat Data Analysis
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = LD_Circadian_Med_IFNy_data) -> cosMod_LD_MIFNy_data
```

```{r}
# Data Conversion to get Individual Points for rLEN Medullary IFNy 
# Rat Model X value is Zeitgeber time, Y value is Concentration in 
# mg of Drug/Content

# Loads Initial Dataset for rLEN Medullary IFNy Rats and Removes Outliers
#outlier_detection(Circadian_Med_IFNy_rLEN) -> out_res_rLENMIFNy
#filter_outliers(Circadian_Med_IFNy_rLEN, 
#                out_res_rLENMIFNy) ->  rLEN_Circadian_Med_IFNy_Filter

# Performs Data Carpentry on Filtered LD IFNy Dataset and Removes 'NA's
rLEN_Circadian_Med_IFNy_Filter %>%
  pivot_longer(cols = c(`0`,`6`,`12`, `18`, `24`), names_to = "ZT", 
               values_to = "Concentration(mg)") %>%
  drop_na(`Concentration(mg)`) %>%
  mutate(ZT = as.numeric(ZT)) %>%
  arrange(ZT) -> rLEN_Circadian_Med_IFNy_Filter
rLEN_Circadian_Med_IFNy_Filter %>%
  # Filter out ZT of 24
  filter(ZT != 24)  -> rLEN_Circadian_Med_IFNy_data

# Creating Cosinor Model for LD IFNy Rat Plots
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = rLEN_Circadian_Med_IFNy_Filter) -> cosMod_rLEN_MIFNy
# Creating Cosinor Model for LD IFNy Rat Plot Data Analysis
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = rLEN_Circadian_Med_IFNy_data) -> cosMod_rLEN_MIFNy_data
```
...
# Future Plot
max(rLEN_Circadian_Med_IFNy_Filter$`Concentration(mg)`, 
    LD_Circadian_Med_IFNy_Filter$`Concentration(mg)`) -> MIFNy_max
ceiling(MIFNy_max / 5) * 5 -> y_peak
```
...
plot_cosinor(cosMod_LD_MIFNy, LD_Circadian_Med_IFNy_Filter, 
             "LD Medullary IFNy", "LD",
             x_breaks = seq(0, 24, by = 6)) -> mod_plot_LD_MIFNy


# Creating Plot for rLEN IFNy Rat Model
plot_cosinor(cosMod_rLEN_MIFNy, rLEN_Circadian_Med_IFNy_Filter, 
             "rLEN Medullary IFNy", "rLEN",
             x_breaks = seq(0, 24, by = 6)) -> mod_plot_rLEN_MIFNy
```


#### Performing Cosinor Analysis for Rat Datasets
```{r}
# Creating Table of Data VOIs for LD Medullary IFNy Rat Model
...
extract_cosinor(cosMod_LD_MIFNy_data, LD_Circadian_Med_IFNy_data,
                "LD") -> LD_extract_output
LD_MIFNy_Results_table <- LD_extract_output$results_table
LD_MIFNy_Results_grob <- LD_extract_output$results_grob


# Creating Table of Data VOIs for rLEN Medullary IFNy Rat Model
## Produces Table of Pertinent Values (Mesor, Amplitude, Acrophase, Bathyphase)
## and Converts Table into Graphical Object for Figure Creation
extract_cosinor(cosMod_rLEN_MIFNy_data, rLEN_Circadian_Med_IFNy_data,
                "rLEN") -> rLEN_extract_output
rLEN_MIFNy_Results_table <- rLEN_extract_output$results_table
rLEN_MIFNy_Results_grob <- rLEN_extract_output$results_grob
```
...
## Creating Table of Zero Amplitude VOIs for LD Medullary IFNy Rat Model

# Produces Table of Pertinent Values (df1, df2, f-value, p-value) and Converts 
# Table into Graphical Object for Figure Creation
zero_amp(cosMod_LD_MIFNy_data, "LD") -> LD_zero_output
LD_MIFNy_amp_test_Results <- LD_zero_output$results_table
LD_MIFNy_amp_test_grob <- LD_zero_output$results_grob


# Produces Table of Pertinent Values (df1, df2, f-value, p-value) and Converts 
# Table into Graphical Object for Figure Creation
zero_amp(cosMod_rLEN_MIFNy_data, "rLEN") -> rLEN_zero_output
rLEN_MIFNy_amp_test_Results <- rLEN_zero_output$results_table
rLEN_MIFNy_amp_test_grob <- rLEN_zero_output$results_grob
```
...
pdf(file.path(plot_dir, "combined_MIFNy_plots.pdf"), width=15, height=6)
grid.arrange(mod_plot_LD_MIFNy, mod_plot_rLEN_MIFNy, ncol = 2)
dev.off()
```
...
tables <- list(LD_MIFNy_Results_grob, LD_MIFNy_amp_test_grob, 
               rLEN_MIFNy_Results_grob, rLEN_MIFNy_amp_test_grob, NULL, NULL)
title_text <- "Comparison of Medullary IFNy LD and rLEN Circadian Disruption Values"
output_file <- file.path(table_dir, "combined_MIFNy_tables.pdf")
fig_width <- 7.5
fig_height <- 10
...
# --- Data Extraction ---
# Extract LD Medullary IFNy (MIFNy) data
LD_MIFNy_acrophase <- LD_MIFNy_Results_table$Values[LD_MIFNy_Results_table$Parameter == "LD Acrophase (Hrs)"]
LD_MIFNy_amplitude <- LD_MIFNy_Results_table$Values[LD_MIFNy_Results_table$Parameter == "LD Amplitude"]

# Extract rLEN Medullary IFNy (MIFNy) data
rLEN_MIFNy_acrophase <- rLEN_MIFNy_Results_table$Values[rLEN_MIFNy_Results_table$Parameter == "rLEN Acrophase (Hrs)"]
rLEN_MIFNy_amplitude <- rLEN_MIFNy_Results_table$Values[rLEN_MIFNy_Results_table$Parameter == "rLEN Amplitude"]

# --- Data Preparation for Combined Plot ---
# Create data frames for each condition
LD_MIFNy_vector_data <- data.frame(
  Condition = "LD",
  acrophase = LD_MIFNy_acrophase,
  amplitude = LD_MIFNy_amplitude
)

rLEN_MIFNy_vector_data <- data.frame(
  Condition = "rLEN",
  acrophase = rLEN_MIFNy_acrophase,
  amplitude = rLEN_MIFNy_amplitude
)

# Combine the two data frames into a single one
combined_MIFNy_vector_data <- rbind(LD_MIFNy_vector_data, rLEN_MIFNy_vector_data) %>%
  arrange(desc(amplitude))


# --- 1. Create the Main Plot ---
max_amplitude <- max(combined_MIFNy_vector_data$amplitude, na.rm = TRUE)
...
main_plot_with_legend <- ggplot(combined_MIFNy_vector_data, aes(color = Condition)) +
  ...
  labs(
    title = "Medullary IFNy Group Acrophase Comparison (LD vs. rLEN)",
...
ggsave(
  filename = file.path(rayleigh_dir, "MedullaryIFNy_Plot.pdf"),  
  plot = final_plot,
