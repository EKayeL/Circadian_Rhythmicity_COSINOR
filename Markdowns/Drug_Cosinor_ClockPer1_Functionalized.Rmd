---
title: "Drug Group Cosinor Clock Per1 Functionalized"
...
extract_cosinor_clock(cosMod_LD_CPer1_data, LD_Circadian_Clock_Per1_data,
...
## Produces Table of Pertinent Values (Mesor, Amplitude, Acrophase, Bathyphase)
## and Converts Table into Graphical Object for Figure Creation
extract_cosinor_clock(cosMod_rLEN_CPer1_data, rLEN_Circadian_Clock_Per1_data,
...
zero_amp(cosMod_LD_CPer1_data, "LD") -> LD_zero_output
...
# Produces Table of Pertinent Values (df1, df2, f-value, p-value) and Converts 
# Table into Graphical Object for Figure Creation
zero_amp(cosMod_rLEN_CPer1_data, "rLEN") -> rLEN_zero_output
...
pdf(file.path(plot_dir, "combined_CPer1_plots.pdf"), width=15, height=6)
...
title_text <- "Comparison of Clock Per1 LD and rLEN Fold Change Values"
output_file <- file.path(table_dir, "combined_CPer1_tables.pdf")
fig_width <- 7.5
fig_height <- 10
...
# --- Data Extraction ---
# Extract LD Clock Per1 (CPer1) data
LD_CPer1_acrophase <- LD_CPer1_Results_table$Values[LD_CPer1_Results_table$Parameter == "LD Acrophase (Hrs)"]
LD_CPer1_amplitude <- LD_CPer1_Results_table$Values[LD_CPer1_Results_table$Parameter == "LD Amplitude"]

# Extract rLEN Clock Per1 (CPer1) data
rLEN_CPer1_acrophase <- rLEN_CPer1_Results_table$Values[rLEN_CPer1_Results_table$Parameter == "rLEN Acrophase (Hrs)"]
rLEN_CPer1_amplitude <- rLEN_CPer1_Results_table$Values[rLEN_CPer1_Results_table$Parameter == "rLEN Amplitude"]

# --- Data Preparation for Combined Plot ---
# Create data frames for each condition
LD_CPer1_vector_data <- data.frame(
  Condition = "LD",
  acrophase = LD_CPer1_acrophase,
  amplitude = LD_CPer1_amplitude
)

rLEN_CPer1_vector_data <- data.frame(
  Condition = "rLEN",
  acrophase = rLEN_CPer1_acrophase,
  amplitude = rLEN_CPer1_amplitude
)

# Combine the two data frames into a single one
combined_CPer1_vector_data <- rbind(LD_CPer1_vector_data, rLEN_CPer1_vector_data) %>%
  arrange(desc(amplitude))


# --- 1. Create the Main Plot ---
max_amplitude <- max(combined_CPer1_vector_data$amplitude, na.rm = TRUE)
...
main_plot_with_legend <- ggplot(combined_CPer1_vector_data, aes(color = Condition)) +
  ...
  labs(
    title = "Clock Per1 Group Acrophase Comparison (LD vs. rLEN)",
...
ggsave(
  filename = file.path(rayleigh_dir, "ClockPer1_Plot.pdf"),  
  plot = final_plot,
