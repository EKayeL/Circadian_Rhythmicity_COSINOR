---
title: "Drug Group Cosinor Thoracic IFNy Functionalized"
...
### Reading, Converting, Analyzing and Plotting Thoracic IFNy 
### Administered Rats 
#### Reading in Thoracic IFNy Administered Rat Data for 
#### LD and rLEN
```{r}
# Reading Thoracic IFNy Administered Rats for LD
readxl::read_excel(path ="../Summary Multiplex Data.xlsx",
                   sheet = "Thoracic",
                    range ="N65:R71") -> LD_Circadian_Thor_IFNy_Filter

# Reading Thoracic IFNy Administered Rats for rLEN
readxl::read_excel(path ="../Summary Multiplex Data.xlsx",
                   sheet = "Thoracic",
                    range ="U65:Y71") -> rLEN_Circadian_Thor_IFNy_Filter
```


#### Converting Data into Usable Format for Extracting VOIs and Plotting
```{r}
# Data Conversion to get Individual Points for LD Thoracic IFNy 
# Rat Model
## X value is Zeitgeber time, Y value is Concentration in mg of Drug/Content

# Loads Initial Dataset for LD Thoracic IFNy Rats and 
# Removes Outliers
#outlier_detection(Circadian_Thor_IFNy_LD) -> out_res_LDTIFNy
#filter_outliers(Circadian_Thor_IFNy_LD, 
#                out_res_LDTIFNy) ->  LD_Circadian_Thor_IFNy_Filter

# Performs Data Carpentry on Filtered LD IFNy Dataset and Removes 'NA's,
## filters out ZT of 24
LD_Circadian_Thor_IFNy_Filter %>%
  pivot_longer(cols = c(`0`,`6`,`12`, `18`, `24`), names_to = "ZT", 
               values_to = "Concentration(mg)") %>%
  drop_na(`Concentration(mg)`) %>%
  mutate(ZT = as.numeric(ZT)) %>%
  arrange(ZT) -> LD_Circadian_Thor_IFNy_Filter
LD_Circadian_Thor_IFNy_Filter %>%
  # Filter out ZT of 24
  filter(ZT != 24)  -> LD_Circadian_Thor_IFNy_data

# Creating Cosinor Model for LD IFNy Rat Plots
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = LD_Circadian_Thor_IFNy_Filter) -> cosMod_LD_TIFNy
# Creating Cosinor Model for LD IFNy Rat Data Analysis
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = LD_Circadian_Thor_IFNy_data) -> cosMod_LD_TIFNy_data
```

```{r}
# Data Conversion to get Individual Points for rLEN Thoracic IFNy 
# Rat Model X value is Zeitgeber time, Y value is Concentration in 
# mg of Drug/Content

# Loads Initial Dataset for rLEN Thoracic IFNy Rats and Removes Outliers
#outlier_detection(Circadian_Thor_IFNy_rLEN) -> out_res_rLENTIFNy
#filter_outliers(Circadian_Thor_IFNy_rLEN, 
#                out_res_rLENTIFNy) ->  rLEN_Circadian_Thor_IFNy_Filter

# Performs Data Carpentry on Filtered LD IFNy Dataset and Removes 'NA's
rLEN_Circadian_Thor_IFNy_Filter %>%
  pivot_longer(cols = c(`0`,`6`,`12`, `18`, `24`), names_to = "ZT", 
               values_to = "Concentration(mg)") %>%
  drop_na(`Concentration(mg)`) %>%
  mutate(ZT = as.numeric(ZT)) %>%
  arrange(ZT) -> rLEN_Circadian_Thor_IFNy_Filter
rLEN_Circadian_Thor_IFNy_Filter %>%
  # Filter out ZT of 24
  filter(ZT != 24)  -> rLEN_Circadian_Thor_IFNy_data

# Creating Cosinor Model for LD IFNy Rat Plots
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = rLEN_Circadian_Thor_IFNy_Filter) -> cosMod_rLEN_TIFNy
# Creating Cosinor Model for LD IFNy Rat Plot Data Analysis
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = rLEN_Circadian_Thor_IFNy_data) -> cosMod_rLEN_TIFNy_data
```
...
# Future Plot
max(rLEN_Circadian_Thor_IFNy_Filter$`Concentration(mg)`, 
    LD_Circadian_Thor_IFNy_Filter$`Concentration(mg)`) -> TIFNy_max
ceiling(TIFNy_max / 5) * 5 -> y_peak
```
...
plot_cosinor(cosMod_LD_TIFNy, LD_Circadian_Thor_IFNy_Filter, 
             "LD Thoracic IFNy", "LD",
             x_breaks = seq(0, 24, by = 6)) -> mod_plot_LD_TIFNy


# Creating Plot for rLEN IFNy Rat Model
plot_cosinor(cosMod_rLEN_TIFNy, rLEN_Circadian_Thor_IFNy_Filter, 
             "rLEN Thoracic IFNy", "rLEN",
             x_breaks = seq(0, 24, by = 6)) -> mod_plot_rLEN_TIFNy
```


#### Performing Cosinor Analysis for Rat Datasets
```{r}
# Creating Table of Data VOIs for LD Thoracic IFNy Rat Model
...
extract_cosinor(cosMod_LD_TIFNy_data, LD_Circadian_Thor_IFNy_data,
                "LD") -> LD_extract_output
LD_TIFNy_Results_table <- LD_extract_output$results_table
LD_TIFNy_Results_grob <- LD_extract_output$results_grob


# Creating Table of Data VOIs for rLEN Thoracic IFNy Rat Model
## Produces Table of Pertinent Values (Mesor, Amplitude, Acrophase, Bathyphase)
## and Converts Table into Graphical Object for Figure Creation
extract_cosinor(cosMod_rLEN_TIFNy_data, rLEN_Circadian_Thor_IFNy_data,
                "rLEN") -> rLEN_extract_output
rLEN_TIFNy_Results_table <- rLEN_extract_output$results_table
rLEN_TIFNy_Results_grob <- rLEN_extract_output$results_grob
```
...
## Creating Table of Zero Amplitude VOIs for LD Thoracic IFNy Rat Model

# Produces Table of Pertinent Values (df1, df2, f-value, p-value) and Converts 
# Table into Graphical Object for Figure Creation
zero_amp(cosMod_LD_TIFNy_data, "LD") -> LD_zero_output
LD_TIFNy_amp_test_Results <- LD_zero_output$results_table
LD_TIFNy_amp_test_grob <- LD_zero_output$results_grob


# Produces Table of Pertinent Values (df1, df2, f-value, p-value) and Converts 
# Table into Graphical Object for Figure Creation
zero_amp(cosMod_rLEN_TIFNy_data, "rLEN") -> rLEN_zero_output
rLEN_TIFNy_amp_test_Results <- rLEN_zero_output$results_table
rLEN_TIFNy_amp_test_grob <- rLEN_zero_output$results_grob
```
...
pdf(file.path(plot_dir, "combined_TIFNy_plots.pdf"), width=15, height=6)
grid.arrange(mod_plot_LD_TIFNy, mod_plot_rLEN_TIFNy, ncol = 2)
dev.off()
```
...
tables <- list(LD_TIFNy_Results_grob, LD_TIFNy_amp_test_grob, 
               rLEN_TIFNy_Results_grob, rLEN_TIFNy_amp_test_grob, NULL, NULL)
title_text <- "Comparison of Thoracic IFNy LD and rLEN Circadian Disruption Values"
output_file <- file.path(table_dir, "combined_TIFNy_tables.pdf")
fig_width <- 7.5
fig_height <- 10
...
# --- Data Extraction ---
# Extract LD Thoracic IFNy (TIFNy) data
LD_TIFNy_acrophase <- LD_TIFNy_Results_table$Values[LD_TIFNy_Results_table$Parameter == "LD Acrophase (Hrs)"]
LD_TIFNy_amplitude <- LD_TIFNy_Results_table$Values[LD_TIFNy_Results_table$Parameter == "LD Amplitude"]

# Extract rLEN Thoracic IFNy (TIFNy) data
rLEN_TIFNy_acrophase <- rLEN_TIFNy_Results_table$Values[rLEN_TIFNy_Results_table$Parameter == "rLEN Acrophase (Hrs)"]
rLEN_TIFNy_amplitude <- rLEN_TIFNy_Results_table$Values[rLEN_TIFNy_Results_table$Parameter == "rLEN Amplitude"]

# --- Data Preparation for Combined Plot ---
# Create data frames for each condition
LD_TIFNy_vector_data <- data.frame(
  Condition = "LD",
  acrophase = LD_TIFNy_acrophase,
  amplitude = LD_TIFNy_amplitude
)

rLEN_TIFNy_vector_data <- data.frame(
  Condition = "rLEN",
  acrophase = rLEN_TIFNy_acrophase,
  amplitude = rLEN_TIFNy_amplitude
)

# Combine the two data frames into a single one
combined_TIFNy_vector_data <- rbind(LD_TIFNy_vector_data, rLEN_TIFNy_vector_data) %>%
  arrange(desc(amplitude))


# --- 1. Create the Main Plot ---
max_amplitude <- max(combined_TIFNy_vector_data$amplitude, na.rm = TRUE)
...
main_plot_with_legend <- ggplot(combined_TIFNy_vector_data, aes(color = Condition)) +
  ...
  labs(
    title = "Thoracic IFNy Group Acrophase Comparison (LD vs. rLEN)",
...
ggsave(
  filename = file.path(rayleigh_dir, "ThoracicIFNy_Plot.pdf"),  
  plot = final_plot,
