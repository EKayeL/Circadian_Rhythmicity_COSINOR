...
# --- Data Extraction ---
# Extract LD Serum Corticosterone (CS) data
LD_CS_acrophase <- LD_CS_Results_table$Values[LD_CS_Results_table$Parameter == "LD Acrophase (Hrs)"]
LD_CS_amplitude <- LD_CS_Results_table$Values[LD_CS_Results_table$Parameter == "LD Amplitude"]

# Extract rLEN Serum Corticosterone (CS) data
rLEN_CS_acrophase <- rLEN_CS_Results_table$Values[rLEN_CS_Results_table$Parameter == "rLEN Acrophase (Hrs)"]
rLEN_CS_amplitude <- rLEN_CS_Results_table$Values[rLEN_CS_Results_table$Parameter == "rLEN Amplitude"]

# --- Data Preparation for Combined Plot ---
# Create data frames for each condition
LD_CS_vector_data <- data.frame(
  Condition = "LD",
  acrophase = LD_CS_acrophase,
  amplitude = LD_CS_amplitude
)

rLEN_CS_vector_data <- data.frame(
  Condition = "rLEN",
  acrophase = rLEN_CS_acrophase,
  amplitude = rLEN_CS_amplitude
)

# Combine the two data frames into a single one
combined_CS_vector_data <- rbind(LD_CS_vector_data, rLEN_CS_vector_data) %>%
  arrange(desc(amplitude))


# --- 1. Create the Main Plot ---
max_amplitude <- max(combined_CS_vector_data$amplitude, na.rm = TRUE)
...
main_plot_with_legend <- ggplot(combined_CS_vector_data, aes(color = Condition)) +
  ...
  labs(
    title = "Serum Corticosterone Group Acrophase Comparison (LD vs. rLEN)",
...
ggsave(
  filename = file.path(rayleigh_dir, "SerumCorticosterone_Plot.pdf"),  
  plot = final_plot,
