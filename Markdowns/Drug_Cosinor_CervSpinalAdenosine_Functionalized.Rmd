---
title: "Drug Group Cosinor Cervical 3-5 Spinal Adenosine Functionalized"
...
### Reading, Analyzing and Plotting Spinal Adenosine Administered Rats
```{r}
# Reading Spinal Adenosine Administered Rats for LD
readxl::read_excel(path ="../Circadian Disruption ZT Data.xlsx",
                   sheet = "Cervical 3-5 Spinal Adenosine",
                    range ="N2:R8") -> Circadian_CervSpinalAden_LD

# Reading Spinal Adenosine Administered Rats for rLEN
readxl::read_excel(path ="../Circadian Disruption ZT Data.xlsx",
                   sheet = "Cervical 3-5 Spinal Adenosine",
                    range ="U2:Y7") -> Circadian_CervSpinalAden_rLEN
```


#### Converting Data into Usable Format for Extracting VOIs and Plotting
``` {r}
# Data Conversion to get Individual Points for LD Spinal Adenosine Rat Model
## X value is Zeitgeber time, Y value is Concentration in mg of Drug/Content

# Loads Initial Dataset for LD Spinal Adenosine Rats and Removes Outliers
outlier_detection(Circadian_CervSpinalAden_LD) -> out_res_LDCAS
filter_outliers(Circadian_CervSpinalAden_LD, 
                out_res_LDCAS) ->  LD_Circadian_CervSpinalAden_Filter

# Performs Data Carpentry on Filtered LD MS Dataset and Removes 'NA's,
## filters out ZT of 24
LD_Circadian_CervSpinalAden_Filter %>%
  pivot_longer(cols = c(`0`,`6`,`12`, `18`, `24`), names_to = "ZT", 
               values_to = "Concentration(mg)") %>%
  drop_na(`Concentration(mg)`) %>%
  mutate(ZT = as.numeric(ZT)) %>%
  arrange(ZT) -> LD_Circadian_CervSpinalAden_Filter
LD_Circadian_CervSpinalAden_Filter %>%
  # Filter out ZT of 24
  filter(ZT != 24)  -> LD_Circadian_CervSpinalAden_data

# Creating Cosinor Model for LD MS Rat Plots
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = LD_Circadian_CervSpinalAden_Filter) -> cosMod_LD_CAS
# Creating Cosinor Model for LD MS Rat Data Analysis
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = LD_Circadian_CervSpinalAden_data) -> cosMod_LD_CAS_data

```

#### Converting Data into Usable Format for Extracting VOIs and Plotting
```{r}
# Data Conversion to get Individual Points for rLEN Spinal Adenosine Rat Model
# X value is Zeitgeber time, Y value is Concentration in mg of Drug/Content

# Loads Initial Dataset for LD Spinal Adenosine Rats and Removes Outliers
outlier_detection(Circadian_CervSpinalAden_rLEN) -> out_res_rLENCAS
filter_outliers(Circadian_CervSpinalAden_rLEN, 
                out_res_rLENCAS) ->  rLEN_Circadian_CervSpinalAden_Filter

# Performs Data Carpentry on Filtered LD MS Dataset and Removes 'NA's
rLEN_Circadian_CervSpinalAden_Filter %>%
  pivot_longer(cols = c(`0`,`6`,`12`, `18`, `24`), names_to = "ZT", 
               values_to = "Concentration(mg)") %>%
  drop_na(`Concentration(mg)`) %>%
  mutate(ZT = as.numeric(ZT)) %>%
  arrange(ZT) -> rLEN_Circadian_CervSpinalAden_Filter
rLEN_Circadian_CervSpinalAden_Filter %>%
  # Filter out ZT of 24
  filter(ZT != 24)  -> rLEN_Circadian_CervSpinalAden_data

# Creating Cosinor Model for LD MS Rat Plots
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = rLEN_Circadian_CervSpinalAden_Filter) -> cosMod_rLEN_CAS
# Creating Cosinor Model for LD MS Rat Plot Data Analysis
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = rLEN_Circadian_CervSpinalAden_data) -> cosMod_rLEN_CAS_data
```


#### Calculating the Y-limits for Plots of Cosinor Fit
```{r}
# Finding the max Y-value Across Both Datsets in Order to Set Y-limits of Future
# Plot
max(rLEN_Circadian_CervSpinalAden_Filter$`Concentration(mg)`, 
    LD_Circadian_CervSpinalAden_Filter$`Concentration(mg)`) -> CAS_max
ceiling(CAS_max / 5) * 5 -> y_peak
```
...
plot_cosinor(cosMod_LD_CAS, LD_Circadian_CervSpinalAden_Filter, 
             "LD Cerv 3-5 Spinal Adenosine", "LD",
             x_breaks = seq(0, 24, by = 6)) -> mod_plot_LD_CAS


# Creating Plot for rLEN CS Rat Model
plot_cosinor(cosMod_rLEN_CAS, rLEN_Circadian_CervSpinalAden_Filter, 
             "rLEN  Cerv 3-5 Spinal Adenosine", "rLEN",
             x_breaks = seq(0, 24, by = 6)) -> mod_plot_rLEN_CAS
```


#### Performing Cosinor Analysis for Rat Datasets
```{r}
# Creating Table of Data VOIs for LD Spinal Adenosine Rat Model
...
extract_cosinor(cosMod_LD_CAS_data, LD_Circadian_CervSpinalAden_data,
                "LD") -> LD_extract_output
LD_CAS_Results_table <- LD_extract_output$results_table
LD_CAS_Results_grob <- LD_extract_output$results_grob


# Creating Table of Data VOIs for rLEN Spinal Adenosine Rat Model
## Produces Table of Pertinent Values (Mesor, Amplitude, Acrophase, Bathyphase)
## and Converts Table into Graphical Object for Figure Creation
extract_cosinor(cosMod_rLEN_CAS_data, rLEN_Circadian_CervSpinalAden_data,
                "rLEN") -> rLEN_extract_output
rLEN_CAS_Results_table <- rLEN_extract_output$results_table
rLEN_CAS_Results_grob <- rLEN_extract_output$results_grob
```
...
## Creating Table of Zero Amplitude VOIs for LD Spinal Adenosine Rat Model

# Produces Table of Pertinent Values (df1, df2, f-value, p-value) and Converts 
# Table into Graphical Object for Figure Creation
zero_amp(cosMod_LD_CAS_data, "LD") -> LD_zero_output
LD_CAS_amp_test_Results <- LD_zero_output$results_table
LD_CAS_amp_test_grob <- LD_zero_output$results_grob


# Produces Table of Pertinent Values (df1, df2, f-value, p-value) and Converts 
# Table into Graphical Object for Figure Creation
zero_amp(cosMod_rLEN_CAS_data, "rLEN") -> rLEN_zero_output
rLEN_CAS_amp_test_Results <- rLEN_zero_output$results_table
rLEN_CAS_amp_test_grob <- rLEN_zero_output$results_grob
```
...
pdf(file.path(plot_dir, "combined_CAS_plots.pdf"), width=15, height=6)
grid.arrange(mod_plot_LD_CAS, mod_plot_rLEN_CAS, ncol = 2)
dev.off()
```
...
tables <- list(LD_CAS_Results_grob, LD_CAS_amp_test_grob, 
               rLEN_CAS_Results_grob, rLEN_CAS_amp_test_grob, NULL, NULL)
title_text <- "Comparison of Cervical 3-5 Spinal Adenosine LD and rLEN Circadian Disruption Values"
output_file <- file.path(table_dir, "combined_CAS_tables.pdf")
fig_width <- 7.5
fig_height <- 10
...
# --- Data Extraction ---
# Extract LD Cervical Spinal Adenosine (CAS) data
LD_CAS_acrophase <- LD_CAS_Results_table$Values[LD_CAS_Results_table$Parameter == "LD Acrophase (Hrs)"]
LD_CAS_amplitude <- LD_CAS_Results_table$Values[LD_CAS_Results_table$Parameter == "LD Amplitude"]

# Extract rLEN Cervical Spinal Adenosine (CAS) data
rLEN_CAS_acrophase <- rLEN_CAS_Results_table$Values[rLEN_CAS_Results_table$Parameter == "rLEN Acrophase (Hrs)"]
rLEN_CAS_amplitude <- rLEN_CAS_Results_table$Values[rLEN_CAS_Results_table$Parameter == "rLEN Amplitude"]

# --- Data Preparation for Combined Plot ---
# Create data frames for each condition
LD_CAS_vector_data <- data.frame(
  Condition = "LD",
  acrophase = LD_CAS_acrophase,
  amplitude = LD_CAS_amplitude
)

rLEN_CAS_vector_data <- data.frame(
  Condition = "rLEN",
  acrophase = rLEN_CAS_acrophase,
  amplitude = rLEN_CAS_amplitude
)

# Combine the two data frames into a single one
combined_CAS_vector_data <- rbind(LD_CAS_vector_data, rLEN_CAS_vector_data) %>%
  arrange(desc(amplitude))


# --- 1. Create the Main Plot ---
max_amplitude <- max(combined_CAS_vector_data$amplitude, na.rm = TRUE)
...
main_plot_with_legend <- ggplot(combined_CAS_vector_data, aes(color = Condition)) +
  ...
  labs(
    title = "Cervical 3-5 Spinal Adenosine Group Acrophase Comparison (LD vs. rLEN)",
...
ggsave(
  filename = file.path(rayleigh_dir, "CervicalSpinalAdenosine_Plot.pdf"),  
  plot = final_plot,
