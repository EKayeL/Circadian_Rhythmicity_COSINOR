...
# --- Data Extraction ---
# Extract LD Cervical IL1a (CIL1a) data
LD_CIL1a_acrophase <- LD_CIL1a_Results_table$Values[LD_CIL1a_Results_table$Parameter == "LD Acrophase (Hrs)"]
LD_CIL1a_amplitude <- LD_CIL1a_Results_table$Values[LD_CIL1a_Results_table$Parameter == "LD Amplitude"]

# Extract rLEN Cervical IL1a (CIL1a) data
rLEN_CIL1a_acrophase <- rLEN_CIL1a_Results_table$Values[rLEN_CIL1a_Results_table$Parameter == "rLEN Acrophase (Hrs)"]
rLEN_CIL1a_amplitude <- rLEN_CIL1a_Results_table$Values[rLEN_CIL1a_Results_table$Parameter == "rLEN Amplitude"]

# --- Data Preparation for Combined Plot ---
# Create data frames for each condition
LD_CIL1a_vector_data <- data.frame(
  Condition = "LD",
  acrophase = LD_CIL1a_acrophase,
  amplitude = LD_CIL1a_amplitude
)

rLEN_CIL1a_vector_data <- data.frame(
  Condition = "rLEN",
  acrophase = rLEN_CIL1a_acrophase,
  amplitude = rLEN_CIL1a_amplitude
)

# Combine the two data frames into a single one
combined_CIL1a_vector_data <- rbind(LD_CIL1a_vector_data, rLEN_CIL1a_vector_data) %>%
  arrange(desc(amplitude))


# --- 1. Create the Main Plot ---
max_amplitude <- max(combined_CIL1a_vector_data$amplitude, na.rm = TRUE)
...
main_plot_with_legend <- ggplot(combined_CIL1a_vector_data, aes(color = Condition)) +
  ...
  labs(
    title = "Cervical IL1a Group Acrophase Comparison (LD vs. rLEN)",
...
ggsave(
  filename = file.path(rayleigh_dir, "CervicalIL1a_Plot.pdf"),  
  plot = final_plot,
