---
title: "Drug Group Cosinor Cervical IL-1b Functionalized"
...
### Reading, Converting, Analyzing and Plotting Cervical IL-1b 
### Administered Rats 
#### Reading in Cervical IL-1b Administered Rat Data for 
#### LD and rLEN
```{r}
# Reading Cervical IL-1b Administered Rats for LD
readxl::read_excel(path ="../Summary Multiplex Data.xlsx",
                   sheet = "Cervical",
                    range ="N11:R17") -> LD_Circadian_Cerv_IL1b_Filter

# Reading Cervical IL-1b Administered Rats for rLEN
readxl::read_excel(path ="../Summary Multiplex Data.xlsx",
                   sheet = "Cervical",
                    range ="U11:Y17") -> rLEN_Circadian_Cerv_IL1b_Filter
```


#### Converting Data into Usable Format for Extracting VOIs and Plotting
```{r}
# Data Conversion to get Individual Points for LD Cervical IL-1b 
# Rat Model
## X value is Zeitgeber time, Y value is Concentration in mg of Drug/Content

# Loads Initial Dataset for LD Cervical IL-1b Rats and 
# Removes Outliers
#outlier_detection(Circadian_Cerv_IL1b_LD) -> out_res_LDCIL1b
#filter_outliers(Circadian_Cerv_IL1b_LD, 
#                out_res_LDCIL1b) ->  LD_Circadian_Cerv_IL1b_Filter

# Performs Data Carpentry on Filtered LD IL1b Dataset and Removes 'NA's,
## filters out ZT of 24
LD_Circadian_Cerv_IL1b_Filter %>%
  pivot_longer(cols = c(`0`,`6`,`12`, `18`, `24`), names_to = "ZT", 
               values_to = "Concentration(mg)") %>%
  drop_na(`Concentration(mg)`) %>%
  mutate(ZT = as.numeric(ZT)) %>%
  arrange(ZT) -> LD_Circadian_Cerv_IL1b_Filter
LD_Circadian_Cerv_IL1b_Filter %>%
  # Filter out ZT of 24
  filter(ZT != 24)  -> LD_Circadian_Cerv_IL1b_data

# Creating Cosinor Model for LD IL1b Rat Plots
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = LD_Circadian_Cerv_IL1b_Filter) -> cosMod_LD_CIL1b
# Creating Cosinor Model for LD IL1b Rat Data Analysis
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = LD_Circadian_Cerv_IL1b_data) -> cosMod_LD_CIL1b_data
```

```{r}
# Data Conversion to get Individual Points for rLEN Cervical IL-1b 
# Rat Model X value is Zeitgeber time, Y value is Concentration in 
# mg of Drug/Content

# Loads Initial Dataset for rLEN Cervical IL-1b Rats and Removes Outliers
#outlier_detection(Circadian_Cerv_IL1b_rLEN) -> out_res_rLENCIL1b
#filter_outliers(Circadian_Cerv_IL1b_rLEN, 
#                out_res_rLENCIL1b) ->  rLEN_Circadian_Cerv_IL1b_Filter

# Performs Data Carpentry on Filtered LD IL1b Dataset and Removes 'NA's
rLEN_Circadian_Cerv_IL1b_Filter %>%
  pivot_longer(cols = c(`0`,`6`,`12`, `18`, `24`), names_to = "ZT", 
               values_to = "Concentration(mg)") %>%
  drop_na(`Concentration(mg)`) %>%
  mutate(ZT = as.numeric(ZT)) %>%
  arrange(ZT) -> rLEN_Circadian_Cerv_IL1b_Filter
rLEN_Circadian_Cerv_IL1b_Filter %>%
  # Filter out ZT of 24
  filter(ZT != 24)  -> rLEN_Circadian_Cerv_IL1b_data

# Creating Cosinor Model for LD IL1b Rat Plots
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = rLEN_Circadian_Cerv_IL1b_Filter) -> cosMod_rLEN_CIL1b
# Creating Cosinor Model for LD IL1b Rat Plot Data Analysis
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = rLEN_Circadian_Cerv_IL1b_data) -> cosMod_rLEN_CIL1b_data
```
...
# Future Plot
max(rLEN_Circadian_Cerv_IL1b_Filter$`Concentration(mg)`, 
    LD_Circadian_Cerv_IL1b_Filter$`Concentration(mg)`) -> CIL1b_max
ceiling(CIL1b_max / 5) * 5 -> y_peak
```
...
plot_cosinor(cosMod_LD_CIL1b, LD_Circadian_Cerv_IL1b_Filter, 
             "LD Cervical IL-1b", "LD",
             x_breaks = seq(0, 24, by = 6)) -> mod_plot_LD_CIL1b


# Creating Plot for rLEN IL1b Rat Model
plot_cosinor(cosMod_rLEN_CIL1b, rLEN_Circadian_Cerv_IL1b_Filter, 
             "rLEN Cervical IL-1b", "rLEN",
             x_breaks = seq(0, 24, by = 6)) -> mod_plot_rLEN_CIL1b
```


#### Performing Cosinor Analysis for Rat Datasets
```{r}
# Creating Table of Data VOIs for LD Cervical IL-1b Rat Model
...
extract_cosinor(cosMod_LD_CIL1b_data, LD_Circadian_Cerv_IL1b_data,
                "LD") -> LD_extract_output
LD_CIL1b_Results_table <- LD_extract_output$results_table
LD_CIL1b_Results_grob <- LD_extract_output$results_grob


# Creating Table of Data VOIs for rLEN Cervical IL-1b Rat Model
## Produces Table of Pertinent Values (Mesor, Amplitude, Acrophase, Bathyphase)
## and Converts Table into Graphical Object for Figure Creation
extract_cosinor(cosMod_rLEN_CIL1b_data, rLEN_Circadian_Cerv_IL1b_data,
                "rLEN") -> rLEN_extract_output
rLEN_CIL1b_Results_table <- rLEN_extract_output$results_table
rLEN_CIL1b_Results_grob <- rLEN_extract_output$results_grob
```
...
## Creating Table of Zero Amplitude VOIs for LD Cervical IL-1b Rat Model

# Produces Table of Pertinent Values (df1, df2, f-value, p-value) and Converts 
# Table into Graphical Object for Figure Creation
zero_amp(cosMod_LD_CIL1b_data, "LD") -> LD_zero_output
LD_CIL1b_amp_test_Results <- LD_zero_output$results_table
LD_CIL1b_amp_test_grob <- LD_zero_output$results_grob


# Produces Table of Pertinent Values (df1, df2, f-value, p-value) and Converts 
# Table into Graphical Object for Figure Creation
zero_amp(cosMod_rLEN_CIL1b_data, "rLEN") -> rLEN_zero_output
rLEN_CIL1b_amp_test_Results <- rLEN_zero_output$results_table
rLEN_CIL1b_amp_test_grob <- rLEN_zero_output$results_grob
```
...
pdf(file.path(plot_dir, "combined_CIL1b_plots.pdf"), width=15, height=6)
grid.arrange(mod_plot_LD_CIL1b, mod_plot_rLEN_CIL1b, ncol = 2)
dev.off()
```
...
tables <- list(LD_CIL1b_Results_grob, LD_CIL1b_amp_test_grob, 
               rLEN_CIL1b_Results_grob, rLEN_CIL1b_amp_test_grob, NULL, NULL)
title_text <- "Comparison of Cervical IL-1b LD and rLEN Circadian Disruption Values"
output_file <- file.path(table_dir, "combined_CIL1b_tables.pdf")
fig_width <- 7.5
fig_height <- 10
...
# --- Data Extraction ---
# Extract LD Cervical IL1b (CIL1b) data
LD_CIL1b_acrophase <- LD_CIL1b_Results_table$Values[LD_CIL1b_Results_table$Parameter == "LD Acrophase (Hrs)"]
LD_CIL1b_amplitude <- LD_CIL1b_Results_table$Values[LD_CIL1b_Results_table$Parameter == "LD Amplitude"]

# Extract rLEN Cervical IL1b (CIL1b) data
rLEN_CIL1b_acrophase <- rLEN_CIL1b_Results_table$Values[rLEN_CIL1b_Results_table$Parameter == "rLEN Acrophase (Hrs)"]
rLEN_CIL1b_amplitude <- rLEN_CIL1b_Results_table$Values[rLEN_CIL1b_Results_table$Parameter == "rLEN Amplitude"]

# --- Data Preparation for Combined Plot ---
# Create data frames for each condition
LD_CIL1b_vector_data <- data.frame(
  Condition = "LD",
  acrophase = LD_CIL1b_acrophase,
  amplitude = LD_CIL1b_amplitude
)

rLEN_CIL1b_vector_data <- data.frame(
  Condition = "rLEN",
  acrophase = rLEN_CIL1b_acrophase,
  amplitude = rLEN_CIL1b_amplitude
)

# Combine the two data frames into a single one
combined_CIL1b_vector_data <- rbind(LD_CIL1b_vector_data, rLEN_CIL1b_vector_data) %>%
  arrange(desc(amplitude))


# --- 1. Create the Main Plot ---
max_amplitude <- max(combined_CIL1b_vector_data$amplitude, na.rm = TRUE)
...
main_plot_with_legend <- ggplot(combined_CIL1b_vector_data, aes(color = Condition)) +
  ...
  labs(
    title = "Cervical IL1b Group Acrophase Comparison (LD vs. rLEN)",
...
ggsave(
  filename = file.path(rayleigh_dir, "CervicalIL1b_Plot.pdf"),  
  plot = final_plot,
