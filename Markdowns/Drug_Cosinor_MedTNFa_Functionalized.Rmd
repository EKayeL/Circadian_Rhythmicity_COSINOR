---
title: "Drug Group Cosinor Medullary TNFa Functionalized"
...
### Reading, Converting, Analyzing and Plotting Medullary TNFa 
### Administered Rats 
#### Reading in Medullary TNFa Administered Rat Data for 
#### LD and rLEN
```{r}
# Reading Medullary TNFa Administered Rats for LD
readxl::read_excel(path ="../Summary Multiplex Data.xlsx",
                   sheet = "Medulla",
                    range ="N74:R80") -> LD_Circadian_Med_TNFa_Filter

# Reading Medullary TNFa Administered Rats for rLEN
readxl::read_excel(path ="../Summary Multiplex Data.xlsx",
                   sheet = "Medulla",
                    range ="U74:Y80") -> rLEN_Circadian_Med_TNFa_Filter
```


#### Converting Data into Usable Format for Extracting VOIs and Plotting
```{r}
# Data Conversion to get Individual Points for LD Medullary TNFa 
# Rat Model
## X value is Zeitgeber time, Y value is Concentration in mg of Drug/Content

# Loads Initial Dataset for LD Medullary TNFa Rats and 
# Removes Outliers
#outlier_detection(Circadian_Med_TNFa_LD) -> out_res_LDMTNFa
#filter_outliers(Circadian_Med_TNFa_LD, 
#                out_res_LDMTNFa) ->  LD_Circadian_Med_TNFa_Filter

# Performs Data Carpentry on Filtered LD TNFa Dataset and Removes 'NA's,
## filters out ZT of 24
LD_Circadian_Med_TNFa_Filter %>%
  pivot_longer(cols = c(`0`,`6`,`12`, `18`, `24`), names_to = "ZT", 
               values_to = "Concentration(mg)") %>%
  drop_na(`Concentration(mg)`) %>%
  mutate(ZT = as.numeric(ZT)) %>%
  arrange(ZT) -> LD_Circadian_Med_TNFa_Filter
LD_Circadian_Med_TNFa_Filter %>%
  # Filter out ZT of 24
  filter(ZT != 24)  -> LD_Circadian_Med_TNFa_data

# Creating Cosinor Model for LD TNFa Rat Plots
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = LD_Circadian_Med_TNFa_Filter) -> cosMod_LD_MTNFa
# Creating Cosinor Model for LD TNFa Rat Data Analysis
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = LD_Circadian_Med_TNFa_data) -> cosMod_LD_MTNFa_data
```

```{r}
# Data Conversion to get Individual Points for rLEN Medullary TNFa 
# Rat Model X value is Zeitgeber time, Y value is Concentration in 
# mg of Drug/Content

# Loads Initial Dataset for rLEN Medullary TNFa Rats and Removes Outliers
#outlier_detection(Circadian_Med_TNFa_rLEN) -> out_res_rLENMTNFa
#filter_outliers(Circadian_Med_TNFa_rLEN, 
#                out_res_rLENMTNFa) ->  rLEN_Circadian_Med_TNFa_Filter

# Performs Data Carpentry on Filtered LD TNFa Dataset and Removes 'NA's
rLEN_Circadian_Med_TNFa_Filter %>%
  pivot_longer(cols = c(`0`,`6`,`12`, `18`, `24`), names_to = "ZT", 
               values_to = "Concentration(mg)") %>%
  drop_na(`Concentration(mg)`) %>%
  mutate(ZT = as.numeric(ZT)) %>%
  arrange(ZT) -> rLEN_Circadian_Med_TNFa_Filter
rLEN_Circadian_Med_TNFa_Filter %>%
  # Filter out ZT of 24
  filter(ZT != 24)  -> rLEN_Circadian_Med_TNFa_data

# Creating Cosinor Model for LD TNFa Rat Plots
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = rLEN_Circadian_Med_TNFa_Filter) -> cosMod_rLEN_MTNFa
# Creating Cosinor Model for LD TNFa Rat Plot Data Analysis
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = rLEN_Circadian_Med_TNFa_data) -> cosMod_rLEN_MTNFa_data
```
...
# Future Plot
max(rLEN_Circadian_Med_TNFa_Filter$`Concentration(mg)`, 
    LD_Circadian_Med_TNFa_Filter$`Concentration(mg)`) -> MTNFa_max
ceiling(MTNFa_max / 5) * 5 -> y_peak
```
...
plot_cosinor(cosMod_LD_MTNFa, LD_Circadian_Med_TNFa_Filter, 
             "LD Medullary TNFa", "LD",
             x_breaks = seq(0, 24, by = 6)) -> mod_plot_LD_MTNFa


# Creating Plot for rLEN TNFa Rat Model
plot_cosinor(cosMod_rLEN_MTNFa, rLEN_Circadian_Med_TNFa_Filter, 
             "rLEN Medullary TNFa", "rLEN",
             x_breaks = seq(0, 24, by = 6)) -> mod_plot_rLEN_MTNFa
```


#### Performing Cosinor Analysis for Rat Datasets
```{r}
# Creating Table of Data VOIs for LD Medullary TNFa Rat Model
...
extract_cosinor(cosMod_LD_MTNFa_data, LD_Circadian_Med_TNFa_data,
                "LD") -> LD_extract_output
LD_MTNFa_Results_table <- LD_extract_output$results_table
LD_MTNFa_Results_grob <- LD_extract_output$results_grob


# Creating Table of Data VOIs for rLEN Medullary TNFa Rat Model
## Produces Table of Pertinent Values (Mesor, Amplitude, Acrophase, Bathyphase)
## and Converts Table into Graphical Object for Figure Creation
extract_cosinor(cosMod_rLEN_MTNFa_data, rLEN_Circadian_Med_TNFa_data,
                "rLEN") -> rLEN_extract_output
rLEN_MTNFa_Results_table <- rLEN_extract_output$results_table
rLEN_MTNFa_Results_grob <- rLEN_extract_output$results_grob
```
...
## Creating Table of Zero Amplitude VOIs for LD Medullary TNFa Rat Model

# Produces Table of Pertinent Values (df1, df2, f-value, p-value) and Converts 
# Table into Graphical Object for Figure Creation
zero_amp(cosMod_LD_MTNFa_data, "LD") -> LD_zero_output
LD_MTNFa_amp_test_Results <- LD_zero_output$results_table
LD_MTNFa_amp_test_grob <- LD_zero_output$results_grob


# Produces Table of Pertinent Values (df1, df2, f-value, p-value) and Converts 
# Table into Graphical Object for Figure Creation
zero_amp(cosMod_rLEN_MTNFa_data, "rLEN") -> rLEN_zero_output
rLEN_MTNFa_amp_test_Results <- rLEN_zero_output$results_table
rLEN_MTNFa_amp_test_grob <- rLEN_zero_output$results_grob
```
...
pdf(file.path(plot_dir, "combined_MTNFa_plots.pdf"), width=15, height=6)
grid.arrange(mod_plot_LD_MTNFa, mod_plot_rLEN_MTNFa, ncol = 2)
dev.off()
```
...
tables <- list(LD_MTNFa_Results_grob, LD_MTNFa_amp_test_grob, 
               rLEN_MTNFa_Results_grob, rLEN_MTNFa_amp_test_grob, NULL, NULL)
title_text <- "Comparison of Medullary TNFa LD and rLEN Circadian Disruption Values"
output_file <- file.path(table_dir, "combined_MTNFa_tables.pdf")
fig_width <- 7.5
fig_height <- 10
...
# --- Data Extraction ---
# Extract LD Medullary TNFa (MTNFa) data
LD_MTNFa_acrophase <- LD_MTNFa_Results_table$Values[LD_MTNFa_Results_table$Parameter == "LD Acrophase (Hrs)"]
LD_MTNFa_amplitude <- LD_MTNFa_Results_table$Values[LD_MTNFa_Results_table$Parameter == "LD Amplitude"]

# Extract rLEN Medullary TNFa (MTNFa) data
rLEN_MTNFa_acrophase <- rLEN_MTNFa_Results_table$Values[rLEN_MTNFa_Results_table$Parameter == "rLEN Acrophase (Hrs)"]
rLEN_MTNFa_amplitude <- rLEN_MTNFa_Results_table$Values[rLEN_MTNFa_Results_table$Parameter == "rLEN Amplitude"]

# --- Data Preparation for Combined Plot ---
# Create data frames for each condition
LD_MTNFa_vector_data <- data.frame(
  Condition = "LD",
  acrophase = LD_MTNFa_acrophase,
  amplitude = LD_MTNFa_amplitude
)

rLEN_MTNFa_vector_data <- data.frame(
  Condition = "rLEN",
  acrophase = rLEN_MTNFa_acrophase,
  amplitude = rLEN_MTNFa_amplitude
)

# Combine the two data frames into a single one
combined_MTNFa_vector_data <- rbind(LD_MTNFa_vector_data, rLEN_MTNFa_vector_data) %>%
  arrange(desc(amplitude))


# --- 1. Create the Main Plot ---
max_amplitude <- max(combined_MTNFa_vector_data$amplitude, na.rm = TRUE)
...
main_plot_with_legend <- ggplot(combined_MTNFa_vector_data, aes(color = Condition)) +
  ...
  labs(
    title = "Medullary TNFa Group Acrophase Comparison (LD vs. rLEN)",
...
ggsave(
  filename = file.path(rayleigh_dir, "MedullaryTNFa_Plot.pdf"),  
  plot = final_plot,
