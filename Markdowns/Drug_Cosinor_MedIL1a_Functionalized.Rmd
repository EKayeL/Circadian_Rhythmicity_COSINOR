---
title: "Drug Group Cosinor Medullary IL-1a Functionalized"
...
### Reading, Converting, Analyzing and Plotting Medullary IL-1a 
### Administered Rats 
#### Reading in Medullary IL-1a Administered Rat Data for 
#### LD and rLEN
```{r}
# Reading Medullary IL-1a Administered Rats for LD
readxl::read_excel(path ="../Summary Multiplex Data.xlsx",
                   sheet = "Medulla",
                    range ="N2:R8") -> LD_Circadian_Med_IL1a_Filter

# Reading Medullary IL-1a Administered Rats for rLEN
readxl::read_excel(path ="../Summary Multiplex Data.xlsx",
                   sheet = "Medulla",
                    range ="U2:Y8") -> rLEN_Circadian_Med_IL1a_Filter
```


#### Converting Data into Usable Format for Extracting VOIs and Plotting
```{r}
# Data Conversion to get Individual Points for LD Medullary IL-1a 
# Rat Model
## X value is Zeitgeber time, Y value is Concentration in mg of Drug/Content

# Loads Initial Dataset for LD Medullary IL-1a Rats and 
# Removes Outliers
#outlier_detection(Circadian_Med_IL1a_LD) -> out_res_LDMIL1a
#filter_outliers(Circadian_Med_IL1a_LD, 
#                out_res_LDMIL1a) ->  LD_Circadian_Med_IL1a_Filter

# Performs Data Carpentry on Filtered LD IL1a Dataset and Removes 'NA's,
## filters out ZT of 24
LD_Circadian_Med_IL1a_Filter %>%
  pivot_longer(cols = c(`0`,`6`,`12`, `18`, `24`), names_to = "ZT", 
               values_to = "Concentration(mg)") %>%
  drop_na(`Concentration(mg)`) %>%
  mutate(ZT = as.numeric(ZT)) %>%
  arrange(ZT) -> LD_Circadian_Med_IL1a_Filter
LD_Circadian_Med_IL1a_Filter %>%
  # Filter out ZT of 24
  filter(ZT != 24)  -> LD_Circadian_Med_IL1a_data

# Creating Cosinor Model for LD IL1a Rat Plots
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = LD_Circadian_Med_IL1a_Filter) -> cosMod_LD_MIL1a
# Creating Cosinor Model for LD IL1a Rat Data Analysis
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = LD_Circadian_Med_IL1a_data) -> cosMod_LD_MIL1a_data
```

```{r}
# Data Conversion to get Individual Points for rLEN Medullary IL-1a 
# Rat Model X value is Zeitgeber time, Y value is Concentration in 
# mg of Drug/Content

# Loads Initial Dataset for rLEN Medullary IL-1a Rats and Removes Outliers
#outlier_detection(Circadian_Med_IL1a_rLEN) -> out_res_rLENMIL1a
#filter_outliers(Circadian_Med_IL1a_rLEN, 
#                out_res_rLENMIL1a) ->  rLEN_Circadian_Med_IL1a_Filter

# Performs Data Carpentry on Filtered LD IL1a Dataset and Removes 'NA's
rLEN_Circadian_Med_IL1a_Filter %>%
  pivot_longer(cols = c(`0`,`6`,`12`, `18`, `24`), names_to = "ZT", 
               values_to = "Concentration(mg)") %>%
  drop_na(`Concentration(mg)`) %>%
  mutate(ZT = as.numeric(ZT)) %>%
  arrange(ZT) -> rLEN_Circadian_Med_IL1a_Filter
rLEN_Circadian_Med_IL1a_Filter %>%
  # Filter out ZT of 24
  filter(ZT != 24)  -> rLEN_Circadian_Med_IL1a_data

# Creating Cosinor Model for LD IL1a Rat Plots
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = rLEN_Circadian_Med_IL1a_Filter) -> cosMod_rLEN_MIL1a
# Creating Cosinor Model for LD IL1a Rat Plot Data Analysis
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = rLEN_Circadian_Med_IL1a_data) -> cosMod_rLEN_MIL1a_data
```
...
# Future Plot
max(rLEN_Circadian_Med_IL1a_Filter$`Concentration(mg)`, 
    LD_Circadian_Med_IL1a_Filter$`Concentration(mg)`) -> MIL1a_max
ceiling(MIL1a_max / 5) * 5 -> y_peak
```
...
plot_cosinor(cosMod_LD_MIL1a, LD_Circadian_Med_IL1a_Filter, 
             "LD Medullary IL-1a", "LD",
             x_breaks = seq(0, 24, by = 6)) -> mod_plot_LD_MIL1a


# Creating Plot for rLEN IL1a Rat Model
plot_cosinor(cosMod_rLEN_MIL1a, rLEN_Circadian_Med_IL1a_Filter, 
             "rLEN Medullary IL-1a", "rLEN",
             x_breaks = seq(0, 24, by = 6)) -> mod_plot_rLEN_MIL1a
```


#### Performing Cosinor Analysis for Rat Datasets
```{r}
# Creating Table of Data VOIs for LD Medullary IL-1a Rat Model
...
extract_cosinor(cosMod_LD_MIL1a_data, LD_Circadian_Med_IL1a_data,
                "LD") -> LD_extract_output
LD_MIL1a_Results_table <- LD_extract_output$results_table
LD_MIL1a_Results_grob <- LD_extract_output$results_grob


# Creating Table of Data VOIs for rLEN Medullary IL-1a Rat Model
## Produces Table of Pertinent Values (Mesor, Amplitude, Acrophase, Bathyphase)
## and Converts Table into Graphical Object for Figure Creation
extract_cosinor(cosMod_rLEN_MIL1a_data, rLEN_Circadian_Med_IL1a_data,
                "rLEN") -> rLEN_extract_output
rLEN_MIL1a_Results_table <- rLEN_extract_output$results_table
rLEN_MIL1a_Results_grob <- rLEN_extract_output$results_grob
```
...
## Creating Table of Zero Amplitude VOIs for LD Medullary IL-1a Rat Model

# Produces Table of Pertinent Values (df1, df2, f-value, p-value) and Converts 
# Table into Graphical Object for Figure Creation
zero_amp(cosMod_LD_MIL1a_data, "LD") -> LD_zero_output
LD_MIL1a_amp_test_Results <- LD_zero_output$results_table
LD_MIL1a_amp_test_grob <- LD_zero_output$results_grob


# Produces Table of Pertinent Values (df1, df2, f-value, p-value) and Converts 
# Table into Graphical Object for Figure Creation
zero_amp(cosMod_rLEN_MIL1a_data, "rLEN") -> rLEN_zero_output
rLEN_MIL1a_amp_test_Results <- rLEN_zero_output$results_table
rLEN_MIL1a_amp_test_grob <- rLEN_zero_output$results_grob
```
...
pdf(file.path(plot_dir, "combined_MIL1a_plots.pdf"), width=15, height=6)
grid.arrange(mod_plot_LD_MIL1a, mod_plot_rLEN_MIL1a, ncol = 2)
dev.off()
```
...
tables <- list(LD_MIL1a_Results_grob, LD_MIL1a_amp_test_grob, 
               rLEN_MIL1a_Results_grob, rLEN_MIL1a_amp_test_grob, NULL, NULL)
title_text <- "Comparison of Medullary IL-1a LD and rLEN Circadian Disruption Values"
output_file <- file.path(table_dir, "combined_MIL1a_tables.pdf")
fig_width <- 7.5
fig_height <- 10
...
# --- Data Extraction ---
# Extract LD Medullary IL1a (MIL1a) data
LD_MIL1a_acrophase <- LD_MIL1a_Results_table$Values[LD_MIL1a_Results_table$Parameter == "LD Acrophase (Hrs)"]
LD_MIL1a_amplitude <- LD_MIL1a_Results_table$Values[LD_MIL1a_Results_table$Parameter == "LD Amplitude"]

# Extract rLEN Medullary IL1a (MIL1a) data
rLEN_MIL1a_acrophase <- rLEN_MIL1a_Results_table$Values[rLEN_MIL1a_Results_table$Parameter == "rLEN Acrophase (Hrs)"]
rLEN_MIL1a_amplitude <- rLEN_MIL1a_Results_table$Values[rLEN_MIL1a_Results_table$Parameter == "rLEN Amplitude"]

# --- Data Preparation for Combined Plot ---
# Create data frames for each condition
LD_MIL1a_vector_data <- data.frame(
  Condition = "LD",
  acrophase = LD_MIL1a_acrophase,
  amplitude = LD_MIL1a_amplitude
)

rLEN_MIL1a_vector_data <- data.frame(
  Condition = "rLEN",
  acrophase = rLEN_MIL1a_acrophase,
  amplitude = rLEN_MIL1a_amplitude
)

# Combine the two data frames into a single one
combined_MIL1a_vector_data <- rbind(LD_MIL1a_vector_data, rLEN_MIL1a_vector_data) %>%
  arrange(desc(amplitude))


# --- 1. Create the Main Plot ---
max_amplitude <- max(combined_MIL1a_vector_data$amplitude, na.rm = TRUE)
...
main_plot_with_legend <- ggplot(combined_MIL1a_vector_data, aes(color = Condition)) +
  ...
  labs(
    title = "Medullary IL1a Group Acrophase Comparison (LD vs. rLEN)",
...
ggsave(
  filename = file.path(rayleigh_dir, "MedullaryIL1a_Plot.pdf"),  
  plot = final_plot,
