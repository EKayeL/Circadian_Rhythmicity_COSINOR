---
title: "Drug Group Cosinor Serum Epinephrine"
...
### Reading, Converting, Analyzing and Plotting Serum Epinephrine 
### Administered Rats 
#### Reading in Serum Epinephrine Administered Rat Data for 
#### LD and rLEN
```{r}
# Reading Serum Epinephrine Administered Rats for LD
readxl::read_excel(path ="../Circadian Disruption ZT Data ABM NEW.xlsx",
                   sheet = "Serum Epinephrine",
                    range ="N2:R10") -> Circadian_SerumEpi_LD

# Reading Serum Epinephrine Administered Rats for rLEN
readxl::read_excel(path ="../Circadian Disruption ZT Data ABM NEW.xlsx",
                   sheet = "Serum Epinephrine",
                    range ="U2:Y12") -> Circadian_SerumEpi_rLEN
```


#### Converting Data into Usable Format for Extracting VOIs and Plotting
```{r}
# Data Conversion to get Individual Points for LD Serum Epinephrine 
# Rat Model
## X value is Zeitgeber time, Y value is Concentration in mg of Drug/Content

# Loads Initial Dataset for LD Serum Epinephrine Rats and 
# Removes Outliers
outlier_detection(Circadian_SerumEpi_LD) -> out_res_LDSE
filter_outliers(Circadian_SerumEpi_LD, 
                out_res_LDSE) ->  LD_Circadian_SerumEpi_Filter

# Performs Data Carpentry on Filtered LD SE Dataset and Removes 'NA's,
## filters out ZT of 24
LD_Circadian_SerumEpi_Filter %>%
  pivot_longer(cols = c(`0`,`6`,`12`, `18`, `24`), names_to = "ZT", 
               values_to = "Concentration(mg)") %>%
  drop_na(`Concentration(mg)`) %>%
  mutate(ZT = as.numeric(ZT)) %>%
  arrange(ZT) -> LD_Circadian_SerumEpi_Filter
LD_Circadian_SerumEpi_Filter %>%
  # Filter out ZT of 24
  filter(ZT != 24)  -> LD_Circadian_SerumEpi_data

# Creating Cosinor Model for LD SE Rat Plots
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = LD_Circadian_SerumEpi_Filter) -> cosMod_LD_SE
# Creating Cosinor Model for LD SE Rat Data Analysis
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = LD_Circadian_SerumEpi_data) -> cosMod_LD_SE_data
```

```{r}
# Data Conversion to get Individual Points for rLEN Serum Epinephrine 
# Rat Model X value is Zeitgeber time, Y value is Concentration in 
# mg of Drug/Content

# Loads Initial Dataset for rLEN Serum Epinephrine Rats and Removes Outliers
outlier_detection(Circadian_SerumEpi_rLEN) -> out_res_rLENSE
filter_outliers(Circadian_SerumEpi_rLEN, 
                out_res_rLENSE) ->  rLEN_Circadian_SerumEpi_Filter

# Performs Data Carpentry on Filtered LD SE Dataset and Removes 'NA's
rLEN_Circadian_SerumEpi_Filter %>%
  pivot_longer(cols = c(`0`,`6`,`12`, `18`, `24`), names_to = "ZT", 
               values_to = "Concentration(mg)") %>%
  drop_na(`Concentration(mg)`) %>%
  mutate(ZT = as.numeric(ZT)) %>%
  arrange(ZT) -> rLEN_Circadian_SerumEpi_Filter
rLEN_Circadian_SerumEpi_Filter %>%
  # Filter out ZT of 24
  filter(ZT != 24)  -> rLEN_Circadian_SerumEpi_data

# Creating Cosinor Model for LD SE Rat Plots
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = rLEN_Circadian_SerumEpi_Filter) -> cosMod_rLEN_SE
# Creating Cosinor Model for LD SE Rat Plot Data Analysis
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = rLEN_Circadian_SerumEpi_data) -> cosMod_rLEN_SE_data
```
...
# Future Plot
max(rLEN_Circadian_SerumEpi_Filter$`Concentration(mg)`, 
    LD_Circadian_SerumEpi_Filter$`Concentration(mg)`) -> SE_max
ceiling(SE_max / 5) * 5 -> y_peak
```
...
# Creating Plot for LD SE Rat Model
plot_cosinor(cosMod_LD_SE, LD_Circadian_SerumEpi_Filter, 
             "LD Serum Epinephrine", "LD",
             x_breaks = seq(0, 24, by = 6)) -> mod_plot_LD_SE


# Creating Plot for rLEN SE Rat Model
plot_cosinor(cosMod_rLEN_SE, rLEN_Circadian_SerumEpi_Filter, 
             "rLEN Serum Epinephrine", "rLEN",
             x_breaks = seq(0, 24, by = 6)) -> mod_plot_rLEN_SE
```


#### Performing Cosinor Analysis for Rat Datasets
```{r}
# Creating Table of Data VOIs for LD Serum Epinephrine Rat Model
...
extract_cosinor(cosMod_LD_SE_data, LD_Circadian_SerumEpi_data,
                "LD") -> LD_extract_output
LD_SE_Results_table <- LD_extract_output$results_table
LD_SE_Results_grob <- LD_extract_output$results_grob


# Creating Table of Data VOIs for rLEN Serum Epinephrine Rat Model
## Produces Table of Pertinent Values (Mesor, Amplitude, Acrophase, Bathyphase)
## and Converts Table into Graphical Object for Figure Creation
extract_cosinor(cosMod_rLEN_SE_data, rLEN_Circadian_SerumEpi_data,
                "rLEN") -> rLEN_extract_output
rLEN_SE_Results_table <- rLEN_extract_output$results_table
rLEN_SE_Results_grob <- rLEN_extract_output$results_grob
```
...
## Creating Table of Zero Amplitude VOIs for LD Serum Epinephrine Rat Model

# Produces Table of Pertinent Values (df1, df2, f-value, p-value) and Converts 
# Table into Graphical Object for Figure Creation
zero_amp(cosMod_LD_SE_data, "LD") -> LD_zero_output
LD_SE_amp_test_Results <- LD_zero_output$results_table
LD_SE_amp_test_grob <- LD_zero_output$results_grob


# Produces Table of Pertinent Values (df1, df2, f-value, p-value) and Converts 
# Table into Graphical Object for Figure Creation
zero_amp(cosMod_rLEN_SE_data, "rLEN") -> rLEN_zero_output
rLEN_SE_amp_test_Results <- rLEN_zero_output$results_table
rLEN_SE_amp_test_grob <- rLEN_zero_output$results_grob
```
...
pdf(file.path(plot_dir, "combined_SE_plots.pdf"), width=15, height=6)
grid.arrange(mod_plot_LD_SE, mod_plot_rLEN_SE, ncol = 2)
dev.off()
```
...
tables <- list(LD_SE_Results_grob, LD_SE_amp_test_grob, 
               rLEN_SE_Results_grob, rLEN_SE_amp_test_grob, NULL, NULL)
title_text <- "Comparison of Serum Epinephrine LD and rLEN Circadian Disruption Values"
output_file <- file.path(table_dir, "combined_SE_tables.pdf")
fig_width <- 7.5
fig_height <- 10
...
# --- Data Extraction ---
# Extract LD Serum Epinephrine (SE) data
LD_SE_acrophase <- LD_SE_Results_table$Values[LD_SE_Results_table$Parameter == "LD Acrophase (Hrs)"]
LD_SE_amplitude <- LD_SE_Results_table$Values[LD_SE_Results_table$Parameter == "LD Amplitude"]

# Extract rLEN Serum Epinephrine (SE) data
rLEN_SE_acrophase <- rLEN_SE_Results_table$Values[rLEN_SE_Results_table$Parameter == "rLEN Acrophase (Hrs)"]
rLEN_SE_amplitude <- rLEN_SE_Results_table$Values[rLEN_SE_Results_table$Parameter == "rLEN Amplitude"]

# --- Data Preparation for Combined Plot ---
# Create data frames for each condition
LD_SE_vector_data <- data.frame(
  Condition = "LD",
  acrophase = LD_SE_acrophase,
  amplitude = LD_SE_amplitude
)

rLEN_SE_vector_data <- data.frame(
  Condition = "rLEN",
  acrophase = rLEN_SE_acrophase,
  amplitude = rLEN_SE_amplitude
)

# Combine the two data frames into a single one
combined_SE_vector_data <- rbind(LD_SE_vector_data, rLEN_SE_vector_data) %>%
  arrange(desc(amplitude))


# --- 1. Create the Main Plot ---
max_amplitude <- max(combined_SE_vector_data$amplitude, na.rm = TRUE)
...
main_plot_with_legend <- ggplot(combined_SE_vector_data, aes(color = Condition)) +
  ...
  labs(
    title = "Serum Epinephrine Group Acrophase Comparison (LD vs. rLEN)",
...
ggsave(
  filename = file.path(rayleigh_dir, "SerumEpinephrine_Plot.pdf"),  
  plot = final_plot,
