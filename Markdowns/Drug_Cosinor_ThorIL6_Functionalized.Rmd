...
# --- Data Extraction ---
# Extract LD Thoracic IL6 (TIL6) data
LD_TIL6_acrophase <- LD_TIL6_Results_table$Values[LD_TIL6_Results_table$Parameter == "LD Acrophase (Hrs)"]
LD_TIL6_amplitude <- LD_TIL6_Results_table$Values[LD_TIL6_Results_table$Parameter == "LD Amplitude"]

# Extract rLEN Thoracic IL6 (TIL6) data
rLEN_TIL6_acrophase <- rLEN_TIL6_Results_table$Values[rLEN_TIL6_Results_table$Parameter == "rLEN Acrophase (Hrs)"]
rLEN_TIL6_amplitude <- rLEN_TIL6_Results_table$Values[rLEN_TIL6_Results_table$Parameter == "rLEN Amplitude"]

# --- Data Preparation for Combined Plot ---
# Create data frames for each condition
LD_TIL6_vector_data <- data.frame(
  Condition = "LD",
  acrophase = LD_TIL6_acrophase,
  amplitude = LD_TIL6_amplitude
)

rLEN_TIL6_vector_data <- data.frame(
  Condition = "rLEN",
  acrophase = rLEN_TIL6_acrophase,
  amplitude = rLEN_TIL6_amplitude
)

# Combine the two data frames into a single one
combined_TIL6_vector_data <- rbind(LD_TIL6_vector_data, rLEN_TIL6_vector_data) %>%
  arrange(desc(amplitude))


# --- 1. Create the Main Plot ---
max_amplitude <- max(combined_TIL6_vector_data$amplitude, na.rm = TRUE)
...
main_plot_with_legend <- ggplot(combined_TIL6_vector_data, aes(color = Condition)) +
  ...
  labs(
    title = "Thoracic IL6 Group Acrophase Comparison (LD vs. rLEN)",
...
ggsave(
  filename = file.path(rayleigh_dir, "ThoracicIL6_Plot.pdf"),  
  plot = final_plot,
