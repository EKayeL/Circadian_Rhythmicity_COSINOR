---
title: "Drug Group Cosinor Thoracic 6-7 Spinal Adenosine Functionalized"
...
### Reading, Converting, Analyzing and Plotting Thoracic 6-7 Spinal Adenosine 
### Administered Rats 
#### Reading in Thoracic 6-7 Spinal Adenosine Administered Rat Data for LD and rLEN
```{r}
# Reading Thoracic 6-7 Spinal Adenosine Administered Rats for LD
readxl::read_excel(path ="../Circadian Disruption ZT Data.xlsx",
                   sheet = "Thoracic 6-7 Spinal Adenosine",
                    range ="N2:R8") -> Circadian_ThorSpinalAden_LD

# Reading Thoracic 6-7 Spinal Adenosine Administered Rats for rLEN
readxl::read_excel(path ="../Circadian Disruption ZT Data.xlsx",
                   sheet = "Thoracic 6-7 Spinal Adenosine",
                    range ="U2:Y7") -> Circadian_ThorSpinalAden_rLEN
```


#### Converting Data into Usable Format for Extracting VOIs and Plotting
```{r}
# Data Conversion to get Individual Points for LD Thoracic 6-7 Spinal Adenosine 
# Rat Model 
## X value is Zeitgeber time, Y value is Concentration in mg of Drug/Content

# Loads Initial Dataset for LD Thoracic 6-7 Spinal Adenosine Rats and 
# Removes Outliers
outlier_detection(Circadian_ThorSpinalAden_LD) -> out_res_LDTAS
filter_outliers(Circadian_ThorSpinalAden_LD, 
                out_res_LDTAS) ->  LD_Circadian_ThorSpinalAden_Filter

# Performs Data Carpentry on Filtered LD MS Dataset and Removes 'NA's,
## filters out ZT of 24
LD_Circadian_ThorSpinalAden_Filter %>%
  pivot_longer(cols = c(`0`,`6`,`12`, `18`, `24`), names_to = "ZT", 
               values_to = "Concentration(mg)") %>%
  drop_na(`Concentration(mg)`) %>%
  mutate(ZT = as.numeric(ZT)) %>%
  arrange(ZT) -> LD_Circadian_ThorSpinalAden_Filter
LD_Circadian_ThorSpinalAden_Filter %>%
  # Filter out ZT of 24
  filter(ZT != 24)  -> LD_Circadian_ThorSpinalAden_data

# Creating Cosinor Model for LD MS Rat Plots
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = LD_Circadian_ThorSpinalAden_Filter) -> cosMod_LD_TAS
# Creating Cosinor Model for LD MS Rat Data Analysis
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = LD_Circadian_ThorSpinalAden_data) -> cosMod_LD_TAS_data
```

```{r}
# Data Conversion to get Individual Points for rLEN Thoracic 6-7 
# Spinal Adenosine 
## Rat Model X value is Zeitgeber time, Y value is Concentration in 
## mg of Drug/Content

# Loads Initial Dataset for rLEN Thoracic 6-7 Spinal Adenosine Rats and 
# Removes Outliers
outlier_detection(Circadian_ThorSpinalAden_rLEN) -> out_res_rLENTAS
filter_outliers(Circadian_ThorSpinalAden_rLEN, 
                out_res_rLENTAS) ->  rLEN_Circadian_ThorSpinalAden_Filter

# Performs Data Carpentry on Filtered LD MS Dataset and Removes 'NA's
rLEN_Circadian_ThorSpinalAden_Filter %>%
  pivot_longer(cols = c(`0`,`6`,`12`, `18`, `24`), names_to = "ZT", 
               values_to = "Concentration(mg)") %>%
  drop_na(`Concentration(mg)`) %>%
  mutate(ZT = as.numeric(ZT)) %>%
  arrange(ZT) -> rLEN_Circadian_ThorSpinalAden_Filter
rLEN_Circadian_ThorSpinalAden_Filter %>%
  # Filter out ZT of 24
  filter(ZT != 24)  -> rLEN_Circadian_ThorSpinalAden_data

# Creating Cosinor Model for LD MS Rat Plots
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = rLEN_Circadian_ThorSpinalAden_Filter) -> cosMod_rLEN_TAS
# Creating Cosinor Model for LD MS Rat Plot Data Analysis
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = rLEN_Circadian_ThorSpinalAden_data) -> cosMod_rLEN_TAS_data
```
...
# Future Plot
max(rLEN_Circadian_ThorSpinalAden_Filter$`Concentration(mg)`, 
    LD_Circadian_ThorSpinalAden_Filter$`Concentration(mg)`) -> CS_max
...
plot_cosinor(cosMod_LD_TAS, LD_Circadian_ThorSpinalAden_Filter, 
             "LD Thoracic 6-7 Spinal Adenosine", "LD",
             x_breaks = seq(0, 24, by = 6)) -> mod_plot_LD_TAS


# Creating Plot for rLEN CS Rat Model
plot_cosinor(cosMod_rLEN_TAS, rLEN_Circadian_ThorSpinalAden_Filter, 
             "rLEN Thoracic 6-7 Spinal Adenosine", "rLEN",
             x_breaks = seq(0, 24, by = 6)) -> mod_plot_rLEN_TAS
```


#### Performing Cosinor Analysis for Rat Datasets
```{r}
# Creating Table of Data VOIs for LD Thoracic 6-7 Spinal Adenosine Rat Model
...
extract_cosinor(cosMod_LD_TAS_data, LD_Circadian_ThorSpinalAden_data,
                "LD") -> LD_extract_output
LD_TAS_Results_table <- LD_extract_output$results_table
LD_TAS_Results_grob <- LD_extract_output$results_grob


# Creating Table of Data VOIs for rLEN Thoracic 6-7 Spinal Adenosine Rat Model
## Produces Table of Pertinent Values (Mesor, Amplitude, Acrophase, Bathyphase)
## and Converts Table into Graphical Object for Figure Creation
extract_cosinor(cosMod_rLEN_TAS_data, rLEN_Circadian_ThorSpinalAden_data,
                "rLEN") -> rLEN_extract_output
rLEN_TAS_Results_table <- rLEN_extract_output$results_table
rLEN_TAS_Results_grob <- rLEN_extract_output$results_grob
```
...
## Creating Table of Zero Amplitude VOIs for LD Thoracic 6-7 Spinal Adenosine Rat Model

# Produces Table of Pertinent Values (df1, df2, f-value, p-value) and Converts 
# Table into Graphical Object for Figure Creation
zero_amp(cosMod_LD_TAS_data, "LD") -> LD_zero_output
LD_TAS_amp_test_Results <- LD_zero_output$results_table
LD_TAS_amp_test_grob <- LD_zero_output$results_grob


# Produces Table of Pertinent Values (df1, df2, f-value, p-value) and Converts 
# Table into Graphical Object for Figure Creation
zero_amp(cosMod_rLEN_TAS_data, "rLEN") -> rLEN_zero_output
rLEN_TAS_amp_test_Results <- rLEN_zero_output$results_table
rLEN_TAS_amp_test_grob <- rLEN_zero_output$results_grob
```
...
pdf(file.path(plot_dir, "combined_TAS_plots.pdf"), width=15, height=6)
grid.arrange(mod_plot_LD_TAS, mod_plot_rLEN_TAS, ncol = 2)
dev.off()
```
...
tables <- list(LD_TAS_Results_grob, LD_TAS_amp_test_grob, 
               rLEN_TAS_Results_grob, rLEN_TAS_amp_test_grob, NULL, NULL)
title_text <- "Comparison of Thoracic 6-7 Spinal Adenosine LD and rLEN Circadian Disruption Values"
output_file <- file.path(table_dir, "combined_TAS_tables.pdf")
fig_width <- 7.5
fig_height <- 10
...
# --- Data Extraction ---
# Extract LD Thoracic Spinal Adenosine (TAS) data
LD_TAS_acrophase <- LD_TAS_Results_table$Values[LD_TAS_Results_table$Parameter == "LD Acrophase (Hrs)"]
LD_TAS_amplitude <- LD_TAS_Results_table$Values[LD_TAS_Results_table$Parameter == "LD Amplitude"]

# Extract rLEN Thoracic Spinal Adenosine (TAS) data
rLEN_TAS_acrophase <- rLEN_TAS_Results_table$Values[rLEN_TAS_Results_table$Parameter == "rLEN Acrophase (Hrs)"]
rLEN_TAS_amplitude <- rLEN_TAS_Results_table$Values[rLEN_TAS_Results_table$Parameter == "rLEN Amplitude"]

# --- Data Preparation for Combined Plot ---
# Create data frames for each condition
LD_TAS_vector_data <- data.frame(
  Condition = "LD",
  acrophase = LD_TAS_acrophase,
  amplitude = LD_TAS_amplitude
)

rLEN_TAS_vector_data <- data.frame(
  Condition = "rLEN",
  acrophase = rLEN_TAS_acrophase,
  amplitude = rLEN_TAS_amplitude
)

# Combine the two data frames into a single one
combined_TAS_vector_data <- rbind(LD_TAS_vector_data, rLEN_TAS_vector_data) %>%
  arrange(desc(amplitude))


# --- 1. Create the Main Plot ---
max_amplitude <- max(combined_TAS_vector_data$amplitude, na.rm = TRUE)
...
main_plot_with_legend <- ggplot(combined_TAS_vector_data, aes(color = Condition)) +
  ...
  labs(
    title = "Thoracic 6-7 Spinal Adenosine Group Acrophase Comparison (LD vs. rLEN)",
...
ggsave(
  filename = file.path(rayleigh_dir, "ThoracicSpinalAdenosine_Plot.pdf"),  
  plot = final_plot,
