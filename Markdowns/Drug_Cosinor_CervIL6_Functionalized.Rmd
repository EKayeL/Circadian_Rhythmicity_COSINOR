...
# --- Data Extraction ---
# Extract LD Cervical IL6 (CIL6) data
LD_CIL6_acrophase <- LD_CIL6_Results_table$Values[LD_CIL6_Results_table$Parameter == "LD Acrophase (Hrs)"]
LD_CIL6_amplitude <- LD_CIL6_Results_table$Values[LD_CIL6_Results_table$Parameter == "LD Amplitude"]

# Extract rLEN Cervical IL6 (CIL6) data
rLEN_CIL6_acrophase <- rLEN_CIL6_Results_table$Values[rLEN_CIL6_Results_table$Parameter == "rLEN Acrophase (Hrs)"]
rLEN_CIL6_amplitude <- rLEN_CIL6_Results_table$Values[rLEN_CIL6_Results_table$Parameter == "rLEN Amplitude"]

# --- Data Preparation for Combined Plot ---
# Create data frames for each condition
LD_CIL6_vector_data <- data.frame(
  Condition = "LD",
  acrophase = LD_CIL6_acrophase,
  amplitude = LD_CIL6_amplitude
)

rLEN_CIL6_vector_data <- data.frame(
  Condition = "rLEN",
  acrophase = rLEN_CIL6_acrophase,
  amplitude = rLEN_CIL6_amplitude
)

# Combine the two data frames into a single one
combined_CIL6_vector_data <- rbind(LD_CIL6_vector_data, rLEN_CIL6_vector_data) %>%
  arrange(desc(amplitude))


# --- 1. Create the Main Plot ---
max_amplitude <- max(combined_CIL6_vector_data$amplitude, na.rm = TRUE)
...
main_plot_with_legend <- ggplot(combined_CIL6_vector_data, aes(color = Condition)) +
  ...
  labs(
    title = "Cervical IL6 Group Acrophase Comparison (LD vs. rLEN)",
...
ggsave(
  filename = file.path(rayleigh_dir, "CervicalIL6_Plot.pdf"),  
  plot = final_plot,
