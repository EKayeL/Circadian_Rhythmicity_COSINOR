...
# --- Data Extraction ---
# Extract LD Medullary Serotonin (SM) data
LD_SM_acrophase <- LD_SM_Results_table$Values[LD_SM_Results_table$Parameter == "LD Acrophase (Hrs)"]
LD_SM_amplitude <- LD_SM_Results_table$Values[LD_SM_Results_table$Parameter == "LD Amplitude"]

# Extract rLEN Medullary Serotonin (SM) data
rLEN_SM_acrophase <- rLEN_SM_Results_table$Values[rLEN_SM_Results_table$Parameter == "rLEN Acrophase (Hrs)"]
rLEN_SM_amplitude <- rLEN_SM_Results_table$Values[rLEN_SM_Results_table$Parameter == "rLEN Amplitude"]

# --- Data Preparation for Combined Plot ---
# Create data frames for each condition
LD_SM_vector_data <- data.frame(
  Condition = "LD",
  acrophase = LD_SM_acrophase,
  amplitude = LD_SM_amplitude
)

rLEN_SM_vector_data <- data.frame(
  Condition = "rLEN",
  acrophase = rLEN_SM_acrophase,
  amplitude = rLEN_SM_amplitude
)

# Combine the two data frames into a single one
combined_SM_vector_data <- rbind(LD_SM_vector_data, rLEN_SM_vector_data) %>%
  arrange(desc(amplitude))


# --- 1. Create the Main Plot ---
max_amplitude <- max(combined_SM_vector_data$amplitude, na.rm = TRUE)
...
main_plot_with_legend <- ggplot(combined_SM_vector_data, aes(color = Condition)) +
  ...
  labs(
    title = "Medullary Serotonin Group Acrophase Comparison (LD vs. rLEN)",
...
ggsave(
  filename = file.path(rayleigh_dir, "MedullarySerotonin_Plot.pdf"),  
  plot = final_plot,
