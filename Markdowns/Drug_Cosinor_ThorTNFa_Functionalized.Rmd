---
title: "Drug Group Cosinor Thoracic TNFa Functionalized"
...
### Reading, Converting, Analyzing and Plotting Thoracic TNFa 
### Administered Rats 
#### Reading in Thoracic TNFa Administered Rat Data for 
#### LD and rLEN
```{r}
# Reading Thoracic TNFa Administered Rats for LD
readxl::read_excel(path ="../Summary Multiplex Data.xlsx",
                   sheet = "Thoracic",
                    range ="N74:R80") -> LD_Circadian_Thor_TNFa_Filter

# Reading Thoracic TNFa Administered Rats for rLEN
readxl::read_excel(path ="../Summary Multiplex Data.xlsx",
                   sheet = "Thoracic",
                    range ="U74:Y80") -> rLEN_Circadian_Thor_TNFa_Filter
```


#### Converting Data into Usable Format for Extracting VOIs and Plotting
```{r}
# Data Conversion to get Individual Points for LD Thoracic TNFa 
# Rat Model
## X value is Zeitgeber time, Y value is Concentration in mg of Drug/Content

# Loads Initial Dataset for LD Thoracic TNFa Rats and 
# Removes Outliers
#outlier_detection(Circadian_Thor_TNFa_LD) -> out_res_LDTTNFa
#filter_outliers(Circadian_Thor_TNFa_LD, 
#                out_res_LDTTNFa) ->  LD_Circadian_Thor_TNFa_Filter

# Performs Data Carpentry on Filtered LD TNFa Dataset and Removes 'NA's,
## filters out ZT of 24
LD_Circadian_Thor_TNFa_Filter %>%
  pivot_longer(cols = c(`0`,`6`,`12`, `18`, `24`), names_to = "ZT", 
               values_to = "Concentration(mg)") %>%
  drop_na(`Concentration(mg)`) %>%
  mutate(ZT = as.numeric(ZT)) %>%
  arrange(ZT) -> LD_Circadian_Thor_TNFa_Filter
LD_Circadian_Thor_TNFa_Filter %>%
  # Filter out ZT of 24
  filter(ZT != 24)  -> LD_Circadian_Thor_TNFa_data

# Creating Cosinor Model for LD TNFa Rat Plots
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = LD_Circadian_Thor_TNFa_Filter) -> cosMod_LD_TTNFa
# Creating Cosinor Model for LD TNFa Rat Data Analysis
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = LD_Circadian_Thor_TNFa_data) -> cosMod_LD_TTNFa_data
```

```{r}
# Data Conversion to get Individual Points for rLEN Thoracic TNFa 
# Rat Model X value is Zeitgeber time, Y value is Concentration in 
# mg of Drug/Content

# Loads Initial Dataset for rLEN Thoracic TNFa Rats and Removes Outliers
#outlier_detection(Circadian_Thor_TNFa_rLEN) -> out_res_rLENTTNFa
#filter_outliers(Circadian_Thor_TNFa_rLEN, 
#                out_res_rLENTTNFa) ->  rLEN_Circadian_Thor_TNFa_Filter

# Performs Data Carpentry on Filtered LD TNFa Dataset and Removes 'NA's
rLEN_Circadian_Thor_TNFa_Filter %>%
  pivot_longer(cols = c(`0`,`6`,`12`, `18`, `24`), names_to = "ZT", 
               values_to = "Concentration(mg)") %>%
  drop_na(`Concentration(mg)`) %>%
  mutate(ZT = as.numeric(ZT)) %>%
  arrange(ZT) -> rLEN_Circadian_Thor_TNFa_Filter
rLEN_Circadian_Thor_TNFa_Filter %>%
  # Filter out ZT of 24
  filter(ZT != 24)  -> rLEN_Circadian_Thor_TNFa_data

# Creating Cosinor Model for LD TNFa Rat Plots
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = rLEN_Circadian_Thor_TNFa_Filter) -> cosMod_rLEN_TTNFa
# Creating Cosinor Model for LD TNFa Rat Plot Data Analysis
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = rLEN_Circadian_Thor_TNFa_data) -> cosMod_rLEN_TTNFa_data
```
...
# Future Plot
max(rLEN_Circadian_Thor_TNFa_Filter$`Concentration(mg)`, 
    LD_Circadian_Thor_TNFa_Filter$`Concentration(mg)`) -> TTNFa_max
ceiling(TTNFa_max / 5) * 5 -> y_peak
```
...
plot_cosinor(cosMod_LD_TTNFa, LD_Circadian_Thor_TNFa_Filter, 
             "LD Thoracic TNFa", "LD",
             x_breaks = seq(0, 24, by = 6)) -> mod_plot_LD_TTNFa


# Creating Plot for rLEN TNFa Rat Model
plot_cosinor(cosMod_rLEN_TTNFa, rLEN_Circadian_Thor_TNFa_Filter, 
             "rLEN Thoracic TNFa", "rLEN",
             x_breaks = seq(0, 24, by = 6)) -> mod_plot_rLEN_TTNFa
```


#### Performing Cosinor Analysis for Rat Datasets
```{r}
# Creating Table of Data VOIs for LD Thoracic TNFa Rat Model
...
extract_cosinor(cosMod_LD_TTNFa_data, LD_Circadian_Thor_TNFa_data,
                "LD") -> LD_extract_output
LD_TTNFa_Results_table <- LD_extract_output$results_table
LD_TTNFa_Results_grob <- LD_extract_output$results_grob


# Creating Table of Data VOIs for rLEN Thoracic TNFa Rat Model
## Produces Table of Pertinent Values (Mesor, Amplitude, Acrophase, Bathyphase)
## and Converts Table into Graphical Object for Figure Creation
extract_cosinor(cosMod_rLEN_TTNFa_data, rLEN_Circadian_Thor_TNFa_data,
                "rLEN") -> rLEN_extract_output
rLEN_TTNFa_Results_table <- rLEN_extract_output$results_table
rLEN_TTNFa_Results_grob <- rLEN_extract_output$results_grob
```
...
## Creating Table of Zero Amplitude VOIs for LD Thoracic TNFa Rat Model

# Produces Table of Pertinent Values (df1, df2, f-value, p-value) and Converts 
# Table into Graphical Object for Figure Creation
zero_amp(cosMod_LD_TTNFa_data, "LD") -> LD_zero_output
LD_TTNFa_amp_test_Results <- LD_zero_output$results_table
LD_TTNFa_amp_test_grob <- LD_zero_output$results_grob


# Produces Table of Pertinent Values (df1, df2, f-value, p-value) and Converts 
# Table into Graphical Object for Figure Creation
zero_amp(cosMod_rLEN_TTNFa_data, "rLEN") -> rLEN_zero_output
rLEN_TTNFa_amp_test_Results <- rLEN_zero_output$results_table
rLEN_TTNFa_amp_test_grob <- rLEN_zero_output$results_grob
```
...
pdf(file.path(plot_dir, "combined_TTNFa_plots.pdf"), width=15, height=6)
grid.arrange(mod_plot_LD_TTNFa, mod_plot_rLEN_TTNFa, ncol = 2)
dev.off()
```
...
tables <- list(LD_TTNFa_Results_grob, LD_TTNFa_amp_test_grob, 
               rLEN_TTNFa_Results_grob, rLEN_TTNFa_amp_test_grob, NULL, NULL)
title_text <- "Comparison of Thoracic TNFa LD and rLEN Circadian Disruption Values"
output_file <- file.path(table_dir, "combined_TTNFa_tables.pdf")
fig_width <- 7.5
fig_height <- 10
...
# --- Data Extraction ---
# Extract LD Thoracic TNFa (TTNFa) data
LD_TTNFa_acrophase <- LD_TTNFa_Results_table$Values[LD_TTNFa_Results_table$Parameter == "LD Acrophase (Hrs)"]
LD_TTNFa_amplitude <- LD_TTNFa_Results_table$Values[LD_TTNFa_Results_table$Parameter == "LD Amplitude"]

# Extract rLEN Thoracic TNFa (TTNFa) data
rLEN_TTNFa_acrophase <- rLEN_TTNFa_Results_table$Values[rLEN_TTNFa_Results_table$Parameter == "rLEN Acrophase (Hrs)"]
rLEN_TTNFa_amplitude <- rLEN_TTNFa_Results_table$Values[rLEN_TTNFa_Results_table$Parameter == "rLEN Amplitude"]

# --- Data Preparation for Combined Plot ---
# Create data frames for each condition
LD_TTNFa_vector_data <- data.frame(
  Condition = "LD",
  acrophase = LD_TTNFa_acrophase,
  amplitude = LD_TTNFa_amplitude
)

rLEN_TTNFa_vector_data <- data.frame(
  Condition = "rLEN",
  acrophase = rLEN_TTNFa_acrophase,
  amplitude = rLEN_TTNFa_amplitude
)

# Combine the two data frames into a single one
combined_TTNFa_vector_data <- rbind(LD_TTNFa_vector_data, rLEN_TTNFa_vector_data) %>%
  arrange(desc(amplitude))


# --- 1. Create the Main Plot ---
max_amplitude <- max(combined_TTNFa_vector_data$amplitude, na.rm = TRUE)
...
main_plot_with_legend <- ggplot(combined_TTNFa_vector_data, aes(color = Condition)) +
  ...
  labs(
    title = "Thoracic TNFa Group Acrophase Comparison (LD vs. rLEN)",
...
ggsave(
  filename = file.path(rayleigh_dir, "ThoracicTNFa_Plot.pdf"),  
  plot = final_plot,
