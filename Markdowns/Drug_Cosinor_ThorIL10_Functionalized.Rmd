---
title: "Drug Group Cosinor Thoracic IL-10 Functionalized"
...
### Reading, Converting, Analyzing and Plotting Thoracic IL-10 
### Administered Rats 
#### Reading in Thoracic IL-10 Administered Rat Data for 
#### LD and rLEN
```{r}
# Reading Thoracic IL-10 Administered Rats for LD
readxl::read_excel(path ="../Summary Multiplex Data.xlsx",
                   sheet = "Thoracic",
                    range ="N47:R53") -> LD_Circadian_Thor_IL10_Filter

# Reading Thoracic IL-10 Administered Rats for rLEN
readxl::read_excel(path ="../Summary Multiplex Data.xlsx",
                   sheet = "Thoracic",
                    range ="U47:Y53") -> rLEN_Circadian_Thor_IL10_Filter
```


#### Converting Data into Usable Format for Extracting VOIs and Plotting
```{r}
# Data Conversion to get Individual Points for LD Thoracic IL-10 
# Rat Model
## X value is Zeitgeber time, Y value is Concentration in mg of Drug/Content

# Loads Initial Dataset for LD Thoracic IL-10 Rats and 
# Removes Outliers
#outlier_detection(Circadian_Thor_IL10_LD) -> out_res_LDTIL10
#filter_outliers(Circadian_Thor_IL10_LD, 
#                out_res_LDTIL10) ->  LD_Circadian_Thor_IL10_Filter

# Performs Data Carpentry on Filtered LD IL10 Dataset and Removes 'NA's,
## filters out ZT of 24
LD_Circadian_Thor_IL10_Filter %>%
  pivot_longer(cols = c(`0`,`6`,`12`, `18`, `24`), names_to = "ZT", 
               values_to = "Concentration(mg)") %>%
  drop_na(`Concentration(mg)`) %>%
  mutate(ZT = as.numeric(ZT)) %>%
  arrange(ZT) -> LD_Circadian_Thor_IL10_Filter
LD_Circadian_Thor_IL10_Filter %>%
  # Filter out ZT of 24
  filter(ZT != 24)  -> LD_Circadian_Thor_IL10_data

# Creating Cosinor Model for LD IL10 Rat Plots
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = LD_Circadian_Thor_IL10_Filter) -> cosMod_LD_TIL10
# Creating Cosinor Model for LD IL10 Rat Data Analysis
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = LD_Circadian_Thor_IL10_data) -> cosMod_LD_TIL10_data
```

```{r}
# Data Conversion to get Individual Points for rLEN Thoracic IL-10 
# Rat Model X value is Zeitgeber time, Y value is Concentration in 
# mg of Drug/Content

# Loads Initial Dataset for rLEN Thoracic IL-10 Rats and Removes Outliers
#outlier_detection(Circadian_Thor_IL10_rLEN) -> out_res_rLENTIL10
#filter_outliers(Circadian_Thor_IL10_rLEN, 
#                out_res_rLENTIL10) ->  rLEN_Circadian_Thor_IL10_Filter

# Performs Data Carpentry on Filtered LD IL10 Dataset and Removes 'NA's
rLEN_Circadian_Thor_IL10_Filter %>%
  pivot_longer(cols = c(`0`,`6`,`12`, `18`, `24`), names_to = "ZT", 
               values_to = "Concentration(mg)") %>%
  drop_na(`Concentration(mg)`) %>%
  mutate(ZT = as.numeric(ZT)) %>%
  arrange(ZT) -> rLEN_Circadian_Thor_IL10_Filter
rLEN_Circadian_Thor_IL10_Filter %>%
  # Filter out ZT of 24
  filter(ZT != 24)  -> rLEN_Circadian_Thor_IL10_data

# Creating Cosinor Model for LD IL10 Rat Plots
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = rLEN_Circadian_Thor_IL10_Filter) -> cosMod_rLEN_TIL10
# Creating Cosinor Model for LD IL10 Rat Plot Data Analysis
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = rLEN_Circadian_Thor_IL10_data) -> cosMod_rLEN_TIL10_data
```
...
# Future Plot
max(rLEN_Circadian_Thor_IL10_Filter$`Concentration(mg)`, 
    LD_Circadian_Thor_IL10_Filter$`Concentration(mg)`) -> TIL10_max
ceiling(TIL10_max / 5) * 5 -> y_peak
```
...
plot_cosinor(cosMod_LD_TIL10, LD_Circadian_Thor_IL10_Filter, 
             "LD Thoracic IL-10", "LD",
             x_breaks = seq(0, 24, by = 6)) -> mod_plot_LD_TIL10


# Creating Plot for rLEN IL10 Rat Model
plot_cosinor(cosMod_rLEN_TIL10, rLEN_Circadian_Thor_IL10_Filter, 
             "rLEN Thoracic IL-10", "rLEN",
             x_breaks = seq(0, 24, by = 6)) -> mod_plot_rLEN_TIL10
```


#### Performing Cosinor Analysis for Rat Datasets
```{r}
# Creating Table of Data VOIs for LD Thoracic IL-10 Rat Model
...
extract_cosinor(cosMod_LD_TIL10_data, LD_Circadian_Thor_IL10_data,
                "LD") -> LD_extract_output
LD_TIL10_Results_table <- LD_extract_output$results_table
LD_TIL10_Results_grob <- LD_extract_output$results_grob


# Creating Table of Data VOIs for rLEN Thoracic IL-10 Rat Model
## Produces Table of Pertinent Values (Mesor, Amplitude, Acrophase, Bathyphase)
## and Converts Table into Graphical Object for Figure Creation
extract_cosinor(cosMod_rLEN_TIL10_data, rLEN_Circadian_Thor_IL10_data,
                "rLEN") -> rLEN_extract_output
rLEN_TIL10_Results_table <- rLEN_extract_output$results_table
rLEN_TIL10_Results_grob <- rLEN_extract_output$results_grob
```
...
## Creating Table of Zero Amplitude VOIs for LD Thoracic IL-10 Rat Model

# Produces Table of Pertinent Values (df1, df2, f-value, p-value) and Converts 
# Table into Graphical Object for Figure Creation
zero_amp(cosMod_LD_TIL10_data, "LD") -> LD_zero_output
LD_TIL10_amp_test_Results <- LD_zero_output$results_table
LD_TIL10_amp_test_grob <- LD_zero_output$results_grob


# Produces Table of Pertinent Values (df1, df2, f-value, p-value) and Converts 
# Table into Graphical Object for Figure Creation
zero_amp(cosMod_rLEN_TIL10_data, "rLEN") -> rLEN_zero_output
rLEN_TIL10_amp_test_Results <- rLEN_zero_output$results_table
rLEN_TIL10_amp_test_grob <- rLEN_zero_output$results_grob
```
...
pdf(file.path(plot_dir, "combined_TIL10_plots.pdf"), width=15, height=6)
grid.arrange(mod_plot_LD_TIL10, mod_plot_rLEN_TIL10, ncol = 2)
dev.off()
```
...
tables <- list(LD_TIL10_Results_grob, LD_TIL10_amp_test_grob, 
               rLEN_TIL10_Results_grob, rLEN_TIL10_amp_test_grob, NULL, NULL)
title_text <- "Comparison of Thoracic IL-10 LD and rLEN Circadian Disruption Values"
output_file <- file.path(table_dir, "combined_TIL10_tables.pdf")
fig_width <- 7.5
fig_height <- 10
...
# --- Data Extraction ---
# Extract LD Thoracic IL10 (TIL10) data
LD_TIL10_acrophase <- LD_TIL10_Results_table$Values[LD_TIL10_Results_table$Parameter == "LD Acrophase (Hrs)"]
LD_TIL10_amplitude <- LD_TIL10_Results_table$Values[LD_TIL10_Results_table$Parameter == "LD Amplitude"]

# Extract rLEN Thoracic IL10 (TIL10) data
rLEN_TIL10_acrophase <- rLEN_TIL10_Results_table$Values[rLEN_TIL10_Results_table$Parameter == "rLEN Acrophase (Hrs)"]
rLEN_TIL10_amplitude <- rLEN_TIL10_Results_table$Values[rLEN_TIL10_Results_table$Parameter == "rLEN Amplitude"]

# --- Data Preparation for Combined Plot ---
# Create data frames for each condition
LD_TIL10_vector_data <- data.frame(
  Condition = "LD",
  acrophase = LD_TIL10_acrophase,
  amplitude = LD_TIL10_amplitude
)

rLEN_TIL10_vector_data <- data.frame(
  Condition = "rLEN",
  acrophase = rLEN_TIL10_acrophase,
  amplitude = rLEN_TIL10_amplitude
)

# Combine the two data frames into a single one
combined_TIL10_vector_data <- rbind(LD_TIL10_vector_data, rLEN_TIL10_vector_data) %>%
  arrange(desc(amplitude))


# --- 1. Create the Main Plot ---
max_amplitude <- max(combined_TIL10_vector_data$amplitude, na.rm = TRUE)
...
main_plot_with_legend <- ggplot(combined_TIL10_vector_data, aes(color = Condition)) +
  ...
  labs(
    title = "Thoracic IL10 Group Acrophase Comparison (LD vs. rLEN)",
...
ggsave(
  filename = file.path(rayleigh_dir, "ThoracicIL10_Plot.pdf"),  
  plot = final_plot,
