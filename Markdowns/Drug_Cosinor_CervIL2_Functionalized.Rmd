...
# --- Data Extraction ---
# Extract LD Cervical IL2 (CIL2) data
LD_CIL2_acrophase <- LD_CIL2_Results_table$Values[LD_CIL2_Results_table$Parameter == "LD Acrophase (Hrs)"]
LD_CIL2_amplitude <- LD_CIL2_Results_table$Values[LD_CIL2_Results_table$Parameter == "LD Amplitude"]

# Extract rLEN Cervical IL2 (CIL2) data
rLEN_CIL2_acrophase <- rLEN_CIL2_Results_table$Values[rLEN_CIL2_Results_table$Parameter == "rLEN Acrophase (Hrs)"]
rLEN_CIL2_amplitude <- rLEN_CIL2_Results_table$Values[rLEN_CIL2_Results_table$Parameter == "rLEN Amplitude"]

# --- Data Preparation for Combined Plot ---
# Create data frames for each condition
LD_CIL2_vector_data <- data.frame(
  Condition = "LD",
  acrophase = LD_CIL2_acrophase,
  amplitude = LD_CIL2_amplitude
)

rLEN_CIL2_vector_data <- data.frame(
  Condition = "rLEN",
  acrophase = rLEN_CIL2_acrophase,
  amplitude = rLEN_CIL2_amplitude
)

# Combine the two data frames into a single one
combined_CIL2_vector_data <- rbind(LD_CIL2_vector_data, rLEN_CIL2_vector_data) %>%
  arrange(desc(amplitude))


# --- 1. Create the Main Plot ---
max_amplitude <- max(combined_CIL2_vector_data$amplitude, na.rm = TRUE)
...
main_plot_with_legend <- ggplot(combined_CIL2_vector_data, aes(color = Condition)) +
  ...
  labs(
    title = "Cervical IL2 Group Acrophase Comparison (LD vs. rLEN)",
...
ggsave(
  filename = file.path(rayleigh_dir, "CervicalIL2_Plot.pdf"),  
  plot = final_plot,
