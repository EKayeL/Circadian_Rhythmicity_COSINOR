...
# --- Data Extraction ---
# Extract LD Medullary IL12 (MIL12) data
LD_MIL12_acrophase <- LD_MIL12_Results_table$Values[LD_MIL12_Results_table$Parameter == "LD Acrophase (Hrs)"]
LD_MIL12_amplitude <- LD_MIL12_Results_table$Values[LD_MIL12_Results_table$Parameter == "LD Amplitude"]

# Extract rLEN Medullary IL12 (MIL12) data
rLEN_MIL12_acrophase <- rLEN_MIL12_Results_table$Values[rLEN_MIL12_Results_table$Parameter == "rLEN Acrophase (Hrs)"]
rLEN_MIL12_amplitude <- rLEN_MIL12_Results_table$Values[rLEN_MIL12_Results_table$Parameter == "rLEN Amplitude"]

# --- Data Preparation for Combined Plot ---
# Create data frames for each condition
LD_MIL12_vector_data <- data.frame(
  Condition = "LD",
  acrophase = LD_MIL12_acrophase,
  amplitude = LD_MIL12_amplitude
)

rLEN_MIL12_vector_data <- data.frame(
  Condition = "rLEN",
  acrophase = rLEN_MIL12_acrophase,
  amplitude = rLEN_MIL12_amplitude
)

# Combine the two data frames into a single one
combined_MIL12_vector_data <- rbind(LD_MIL12_vector_data, rLEN_MIL12_vector_data) %>%
  arrange(desc(amplitude))


# --- 1. Create the Main Plot ---
max_amplitude <- max(combined_MIL12_vector_data$amplitude, na.rm = TRUE)
...
main_plot_with_legend <- ggplot(combined_MIL12_vector_data, aes(color = Condition)) +
  ...
  labs(
    title = "Medullary IL12 Group Acrophase Comparison (LD vs. rLEN)",
...
ggsave(
  filename = file.path(rayleigh_dir, "MedullaryIL12_Plot.pdf"),  
  plot = final_plot,
