...
# --- Data Extraction ---
# Extract LD Thoracic IL1a (TIL1a) data
LD_TIL1a_acrophase <- LD_TIL1a_Results_table$Values[LD_TIL1a_Results_table$Parameter == "LD Acrophase (Hrs)"]
LD_TIL1a_amplitude <- LD_TIL1a_Results_table$Values[LD_TIL1a_Results_table$Parameter == "LD Amplitude"]

# Extract rLEN Thoracic IL1a (TIL1a) data
rLEN_TIL1a_acrophase <- rLEN_TIL1a_Results_table$Values[rLEN_TIL1a_Results_table$Parameter == "rLEN Acrophase (Hrs)"]
rLEN_TIL1a_amplitude <- rLEN_TIL1a_Results_table$Values[rLEN_TIL1a_Results_table$Parameter == "rLEN Amplitude"]

# --- Data Preparation for Combined Plot ---
# Create data frames for each condition
LD_TIL1a_vector_data <- data.frame(
  Condition = "LD",
  acrophase = LD_TIL1a_acrophase,
  amplitude = LD_TIL1a_amplitude
)

rLEN_TIL1a_vector_data <- data.frame(
  Condition = "rLEN",
  acrophase = rLEN_TIL1a_acrophase,
  amplitude = rLEN_TIL1a_amplitude
)

# Combine the two data frames into a single one
combined_TIL1a_vector_data <- rbind(LD_TIL1a_vector_data, rLEN_TIL1a_vector_data) %>%
  arrange(desc(amplitude))


# --- 1. Create the Main Plot ---
max_amplitude <- max(combined_TIL1a_vector_data$amplitude, na.rm = TRUE)
...
main_plot_with_legend <- ggplot(combined_TIL1a_vector_data, aes(color = Condition)) +
  ...
  labs(
    title = "Thoracic IL1a Group Acrophase Comparison (LD vs. rLEN)",
...
ggsave(
  filename = file.path(rayleigh_dir, "ThoracicIL1a_Plot.pdf"),  
  plot = final_plot,
