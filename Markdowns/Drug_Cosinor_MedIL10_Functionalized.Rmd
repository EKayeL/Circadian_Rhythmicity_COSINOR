...
# --- Data Extraction ---
# Extract LD Medullary IL10 (MIL10) data
LD_MIL10_acrophase <- LD_MIL10_Results_table$Values[LD_MIL10_Results_table$Parameter == "LD Acrophase (Hrs)"]
LD_MIL10_amplitude <- LD_MIL10_Results_table$Values[LD_MIL10_Results_table$Parameter == "LD Amplitude"]

# Extract rLEN Medullary IL10 (MIL10) data
rLEN_MIL10_acrophase <- rLEN_MIL10_Results_table$Values[rLEN_MIL10_Results_table$Parameter == "rLEN Acrophase (Hrs)"]
rLEN_MIL10_amplitude <- rLEN_MIL10_Results_table$Values[rLEN_MIL10_Results_table$Parameter == "rLEN Amplitude"]

# --- Data Preparation for Combined Plot ---
# Create data frames for each condition
LD_MIL10_vector_data <- data.frame(
  Condition = "LD",
  acrophase = LD_MIL10_acrophase,
  amplitude = LD_MIL10_amplitude
)

rLEN_MIL10_vector_data <- data.frame(
  Condition = "rLEN",
  acrophase = rLEN_MIL10_acrophase,
  amplitude = rLEN_MIL10_amplitude
)

# Combine the two data frames into a single one
combined_MIL10_vector_data <- rbind(LD_MIL10_vector_data, rLEN_MIL10_vector_data) %>%
  arrange(desc(amplitude))


# --- 1. Create the Main Plot ---
max_amplitude <- max(combined_MIL10_vector_data$amplitude, na.rm = TRUE)
...
main_plot_with_legend <- ggplot(combined_MIL10_vector_data, aes(color = Condition)) +
  ...
  labs(
    title = "Medullary IL10 Group Acrophase Comparison (LD vs. rLEN)",
...
ggsave(
  filename = file.path(rayleigh_dir, "MedullaryIL10_Plot.pdf"),  
  plot = final_plot,
