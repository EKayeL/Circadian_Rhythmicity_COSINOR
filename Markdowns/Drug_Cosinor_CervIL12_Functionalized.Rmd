---
title: "Drug Group Cosinor Cervical IL-12 Functionalized"
...
### Reading, Converting, Analyzing and Plotting Cervical IL-12 
### Administered Rats 
#### Reading in Cervical IL-12 Administered Rat Data for 
#### LD and rLEN
```{r}
# Reading Cervical IL-12 Administered Rats for LD
readxl::read_excel(path ="../Summary Multiplex Data.xlsx",
                   sheet = "Cervical",
                    range ="N56:R61") -> LD_Circadian_Cerv_IL12_Filter

# Reading Cervical IL-12 Administered Rats for rLEN
readxl::read_excel(path ="../Summary Multiplex Data.xlsx",
                   sheet = "Cervical",
                    range ="U56:Y61") -> rLEN_Circadian_Cerv_IL12_Filter
```


#### Converting Data into Usable Format for Extracting VOIs and Plotting
```{r}
# Data Conversion to get Individual Points for LD Cervical IL-12 
# Rat Model
## X value is Zeitgeber time, Y value is Concentration in mg of Drug/Content

# Loads Initial Dataset for LD Cervical IL-12 Rats and 
# Removes Outliers
#outlier_detection(Circadian_Cerv_IL12_LD) -> out_res_LDCIL12
#filter_outliers(Circadian_Cerv_IL12_LD, 
#                out_res_LDCIL12) ->  LD_Circadian_Cerv_IL12_Filter

# Performs Data Carpentry on Filtered LD IL12 Dataset and Removes 'NA's,
## filters out ZT of 24
LD_Circadian_Cerv_IL12_Filter %>%
  pivot_longer(cols = c(`0`,`6`,`12`, `18`, `24`), names_to = "ZT", 
               values_to = "Concentration(mg)") %>%
  drop_na(`Concentration(mg)`) %>%
  mutate(ZT = as.numeric(ZT)) %>%
  arrange(ZT) -> LD_Circadian_Cerv_IL12_Filter
LD_Circadian_Cerv_IL12_Filter %>%
  # Filter out ZT of 24
  filter(ZT != 24)  -> LD_Circadian_Cerv_IL12_data

# Creating Cosinor Model for LD IL12 Rat Plots
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = LD_Circadian_Cerv_IL12_Filter) -> cosMod_LD_CIL12
# Creating Cosinor Model for LD IL12 Rat Data Analysis
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = LD_Circadian_Cerv_IL12_data) -> cosMod_LD_CIL12_data
```

```{r}
# Data Conversion to get Individual Points for rLEN Cervical IL-12 
# Rat Model X value is Zeitgeber time, Y value is Concentration in 
# mg of Drug/Content

# Loads Initial Dataset for rLEN Cervical IL-12 Rats and Removes Outliers
#outlier_detection(Circadian_Cerv_IL12_rLEN) -> out_res_rLENCIL12
#filter_outliers(Circadian_Cerv_IL12_rLEN, 
#                out_res_rLENCIL12) ->  rLEN_Circadian_Cerv_IL12_Filter

# Performs Data Carpentry on Filtered LD IL12 Dataset and Removes 'NA's
rLEN_Circadian_Cerv_IL12_Filter %>%
  pivot_longer(cols = c(`0`,`6`,`12`, `18`, `24`), names_to = "ZT", 
               values_to = "Concentration(mg)") %>%
  drop_na(`Concentration(mg)`) %>%
  mutate(ZT = as.numeric(ZT)) %>%
  arrange(ZT) -> rLEN_Circadian_Cerv_IL12_Filter
rLEN_Circadian_Cerv_IL12_Filter %>%
  # Filter out ZT of 24
  filter(ZT != 24)  -> rLEN_Circadian_Cerv_IL12_data

# Creating Cosinor Model for LD IL12 Rat Plots
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = rLEN_Circadian_Cerv_IL12_Filter) -> cosMod_rLEN_CIL12
# Creating Cosinor Model for LD IL12 Rat Plot Data Analysis
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = rLEN_Circadian_Cerv_IL12_data) -> cosMod_rLEN_CIL12_data
```
...
# Future Plot
max(rLEN_Circadian_Cerv_IL12_Filter$`Concentration(mg)`, 
    LD_Circadian_Cerv_IL12_Filter$`Concentration(mg)`) -> CIL12_max
ceiling(CIL12_max / 5) * 5 -> y_peak
```
...
plot_cosinor(cosMod_LD_CIL12, LD_Circadian_Cerv_IL12_Filter, 
             "LD Cervical IL-12", "LD",
             x_breaks = seq(0, 24, by = 6)) -> mod_plot_LD_CIL12


# Creating Plot for rLEN IL12 Rat Model
plot_cosinor(cosMod_rLEN_CIL12, rLEN_Circadian_Cerv_IL12_Filter, 
             "rLEN Cervical IL-12", "rLEN",
             x_breaks = seq(0, 24, by = 6)) -> mod_plot_rLEN_CIL12
```


#### Performing Cosinor Analysis for Rat Datasets
```{r}
# Creating Table of Data VOIs for LD Cervical IL-12 Rat Model
...
extract_cosinor(cosMod_LD_CIL12_data, LD_Circadian_Cerv_IL12_data,
                "LD") -> LD_extract_output
LD_CIL12_Results_table <- LD_extract_output$results_table
LD_CIL12_Results_grob <- LD_extract_output$results_grob


# Creating Table of Data VOIs for rLEN Cervical IL-12 Rat Model
## Produces Table of Pertinent Values (Mesor, Amplitude, Acrophase, Bathyphase)
## and Converts Table into Graphical Object for Figure Creation
extract_cosinor(cosMod_rLEN_CIL12_data, rLEN_Circadian_Cerv_IL12_data,
                "rLEN") -> rLEN_extract_output
rLEN_CIL12_Results_table <- rLEN_extract_output$results_table
rLEN_CIL12_Results_grob <- rLEN_extract_output$results_grob
```
...
## Creating Table of Zero Amplitude VOIs for LD Cervical IL-12 Rat Model

# Produces Table of Pertinent Values (df1, df2, f-value, p-value) and Converts 
# Table into Graphical Object for Figure Creation
zero_amp(cosMod_LD_CIL12_data, "LD") -> LD_zero_output
LD_CIL12_amp_test_Results <- LD_zero_output$results_table
LD_CIL12_amp_test_grob <- LD_zero_output$results_grob


# Produces Table of Pertinent Values (df1, df2, f-value, p-value) and Converts 
# Table into Graphical Object for Figure Creation
zero_amp(cosMod_rLEN_CIL12_data, "rLEN") -> rLEN_zero_output
rLEN_CIL12_amp_test_Results <- rLEN_zero_output$results_table
rLEN_CIL12_amp_test_grob <- rLEN_zero_output$results_grob
```
...
pdf(file.path(plot_dir, "combined_CIL12_plots.pdf"), width=15, height=6)
grid.arrange(mod_plot_LD_CIL12, mod_plot_rLEN_CIL12, ncol = 2)
dev.off()
```
...
tables <- list(LD_CIL12_Results_grob, LD_CIL12_amp_test_grob, 
               rLEN_CIL12_Results_grob, rLEN_CIL12_amp_test_grob, NULL, NULL)
title_text <- "Comparison of Cervical IL-12 LD and rLEN Circadian Disruption Values"
output_file <- file.path(table_dir, "combined_CIL12_tables.pdf")
fig_width <- 7.5
fig_height <- 10
...
# --- Data Extraction ---
# Extract LD Cervical IL12 (CIL12) data
LD_CIL12_acrophase <- LD_CIL12_Results_table$Values[LD_CIL12_Results_table$Parameter == "LD Acrophase (Hrs)"]
LD_CIL12_amplitude <- LD_CIL12_Results_table$Values[LD_CIL12_Results_table$Parameter == "LD Amplitude"]

# Extract rLEN Cervical IL12 (CIL12) data
rLEN_CIL12_acrophase <- rLEN_CIL12_Results_table$Values[rLEN_CIL12_Results_table$Parameter == "rLEN Acrophase (Hrs)"]
rLEN_CIL12_amplitude <- rLEN_CIL12_Results_table$Values[rLEN_CIL12_Results_table$Parameter == "rLEN Amplitude"]

# --- Data Preparation for Combined Plot ---
# Create data frames for each condition
LD_CIL12_vector_data <- data.frame(
  Condition = "LD",
  acrophase = LD_CIL12_acrophase,
  amplitude = LD_CIL12_amplitude
)

rLEN_CIL12_vector_data <- data.frame(
  Condition = "rLEN",
  acrophase = rLEN_CIL12_acrophase,
  amplitude = rLEN_CIL12_amplitude
)

# Combine the two data frames into a single one
combined_CIL12_vector_data <- rbind(LD_CIL12_vector_data, rLEN_CIL12_vector_data) %>%
  arrange(desc(amplitude))


# --- 1. Create the Main Plot ---
max_amplitude <- max(combined_CIL12_vector_data$amplitude, na.rm = TRUE)
...
main_plot_with_legend <- ggplot(combined_CIL12_vector_data, aes(color = Condition)) +
  ...
  labs(
    title = "Cervical IL12 Group Acrophase Comparison (LD vs. rLEN)",
...
ggsave(
  filename = file.path(rayleigh_dir, "CervicalIL12_Plot.pdf"),  
  plot = final_plot,
