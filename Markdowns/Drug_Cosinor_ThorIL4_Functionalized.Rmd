---
title: "Drug Group Cosinor Thoracic IL-4 Functionalized"
...
### Reading, Converting, Analyzing and Plotting Thoracic IL-4 
### Administered Rats 
#### Reading in Thoracic IL-4 Administered Rat Data for 
#### LD and rLEN
```{r}
# Reading Thoracic IL-4 Administered Rats for LD
readxl::read_excel(path ="../Summary Multiplex Data.xlsx",
                   sheet = "Thoracic",
                    range ="N29:R35") -> LD_Circadian_Thor_IL4_Filter

# Reading Thoracic IL-4 Administered Rats for rLEN
readxl::read_excel(path ="../Summary Multiplex Data.xlsx",
                   sheet = "Thoracic",
                    range ="U29:Y35") -> rLEN_Circadian_Thor_IL4_Filter
```


#### Converting Data into Usable Format for Extracting VOIs and Plotting
```{r}
# Data Conversion to get Individual Points for LD Thoracic IL-4 
# Rat Model
## X value is Zeitgeber time, Y value is Concentration in mg of Drug/Content

# Loads Initial Dataset for LD Thoracic IL-4 Rats and 
# Removes Outliers
#outlier_detection(Circadian_Thor_IL4_LD) -> out_res_LDTIL4
#filter_outliers(Circadian_Thor_IL4_LD, 
#                out_res_LDTIL4) ->  LD_Circadian_Thor_IL4_Filter

# Performs Data Carpentry on Filtered LD IL4 Dataset and Removes 'NA's,
## filters out ZT of 24
LD_Circadian_Thor_IL4_Filter %>%
  pivot_longer(cols = c(`0`,`6`,`12`, `18`, `24`), names_to = "ZT", 
               values_to = "Concentration(mg)") %>%
  drop_na(`Concentration(mg)`) %>%
  mutate(ZT = as.numeric(ZT)) %>%
  arrange(ZT) -> LD_Circadian_Thor_IL4_Filter
LD_Circadian_Thor_IL4_Filter %>%
  # Filter out ZT of 24
  filter(ZT != 24)  -> LD_Circadian_Thor_IL4_data

# Creating Cosinor Model for LD IL4 Rat Plots
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = LD_Circadian_Thor_IL4_Filter) -> cosMod_LD_TIL4
# Creating Cosinor Model for LD IL4 Rat Data Analysis
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = LD_Circadian_Thor_IL4_data) -> cosMod_LD_TIL4_data
```

```{r}
# Data Conversion to get Individual Points for rLEN Thoracic IL-4 
# Rat Model X value is Zeitgeber time, Y value is Concentration in 
# mg of Drug/Content

# Loads Initial Dataset for rLEN Thoracic IL-4 Rats and Removes Outliers
#outlier_detection(Circadian_Thor_IL4_rLEN) -> out_res_rLENTIL4
#filter_outliers(Circadian_Thor_IL4_rLEN, 
#                out_res_rLENTIL4) ->  rLEN_Circadian_Thor_IL4_Filter

# Performs Data Carpentry on Filtered LD IL4 Dataset and Removes 'NA's
rLEN_Circadian_Thor_IL4_Filter %>%
  pivot_longer(cols = c(`0`,`6`,`12`, `18`, `24`), names_to = "ZT", 
               values_to = "Concentration(mg)") %>%
  drop_na(`Concentration(mg)`) %>%
  mutate(ZT = as.numeric(ZT)) %>%
  arrange(ZT) -> rLEN_Circadian_Thor_IL4_Filter
rLEN_Circadian_Thor_IL4_Filter %>%
  # Filter out ZT of 24
  filter(ZT != 24)  -> rLEN_Circadian_Thor_IL4_data

# Creating Cosinor Model for LD IL4 Rat Plots
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = rLEN_Circadian_Thor_IL4_Filter) -> cosMod_rLEN_TIL4
# Creating Cosinor Model for LD IL4 Rat Plot Data Analysis
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = rLEN_Circadian_Thor_IL4_data) -> cosMod_rLEN_TIL4_data
```
...
# Future Plot
max(rLEN_Circadian_Thor_IL4_Filter$`Concentration(mg)`, 
    LD_Circadian_Thor_IL4_Filter$`Concentration(mg)`) -> TIL4_max
ceiling(TIL4_max / 5) * 5 -> y_peak
```
...
plot_cosinor(cosMod_LD_TIL4, LD_Circadian_Thor_IL4_Filter, 
             "LD Thoracic IL-4", "LD",
             x_breaks = seq(0, 24, by = 6)) -> mod_plot_LD_TIL4


# Creating Plot for rLEN IL4 Rat Model
plot_cosinor(cosMod_rLEN_TIL4, rLEN_Circadian_Thor_IL4_Filter, 
             "rLEN Thoracic IL-4", "rLEN",
             x_breaks = seq(0, 24, by = 6)) -> mod_plot_rLEN_TIL4
```


#### Performing Cosinor Analysis for Rat Datasets
```{r}
# Creating Table of Data VOIs for LD Thoracic IL-4 Rat Model
...
extract_cosinor(cosMod_LD_TIL4_data, LD_Circadian_Thor_IL4_data,
                "LD") -> LD_extract_output
LD_TIL4_Results_table <- LD_extract_output$results_table
LD_TIL4_Results_grob <- LD_extract_output$results_grob


# Creating Table of Data VOIs for rLEN Thoracic IL-4 Rat Model
## Produces Table of Pertinent Values (Mesor, Amplitude, Acrophase, Bathyphase)
## and Converts Table into Graphical Object for Figure Creation
extract_cosinor(cosMod_rLEN_TIL4_data, rLEN_Circadian_Thor_IL4_data,
                "rLEN") -> rLEN_extract_output
rLEN_TIL4_Results_table <- rLEN_extract_output$results_table
rLEN_TIL4_Results_grob <- rLEN_extract_output$results_grob
```
...
## Creating Table of Zero Amplitude VOIs for LD Thoracic IL-4 Rat Model

# Produces Table of Pertinent Values (df1, df2, f-value, p-value) and Converts 
# Table into Graphical Object for Figure Creation
zero_amp(cosMod_LD_TIL4_data, "LD") -> LD_zero_output
LD_TIL4_amp_test_Results <- LD_zero_output$results_table
LD_TIL4_amp_test_grob <- LD_zero_output$results_grob


# Produces Table of Pertinent Values (df1, df2, f-value, p-value) and Converts 
# Table into Graphical Object for Figure Creation
zero_amp(cosMod_rLEN_TIL4_data, "rLEN") -> rLEN_zero_output
rLEN_TIL4_amp_test_Results <- rLEN_zero_output$results_table
rLEN_TIL4_amp_test_grob <- rLEN_zero_output$results_grob
```
...
pdf(file.path(plot_dir, "combined_TIL4_plots.pdf"), width=15, height=6)
grid.arrange(mod_plot_LD_TIL4, mod_plot_rLEN_TIL4, ncol = 2)
dev.off()
```
...
tables <- list(LD_TIL4_Results_grob, LD_TIL4_amp_test_grob, 
               rLEN_TIL4_Results_grob, rLEN_TIL4_amp_test_grob, NULL, NULL)
title_text <- "Comparison of Thoracic IL-4 LD and rLEN Circadian Disruption Values"
output_file <- file.path(table_dir, "combined_TIL4_tables.pdf")
fig_width <- 7.5
fig_height <- 10
...
# --- Data Extraction ---
# Extract LD Thoracic IL4 (TIL4) data
LD_TIL4_acrophase <- LD_TIL4_Results_table$Values[LD_TIL4_Results_table$Parameter == "LD Acrophase (Hrs)"]
LD_TIL4_amplitude <- LD_TIL4_Results_table$Values[LD_TIL4_Results_table$Parameter == "LD Amplitude"]

# Extract rLEN Thoracic IL4 (TIL4) data
rLEN_TIL4_acrophase <- rLEN_TIL4_Results_table$Values[rLEN_TIL4_Results_table$Parameter == "rLEN Acrophase (Hrs)"]
rLEN_TIL4_amplitude <- rLEN_TIL4_Results_table$Values[rLEN_TIL4_Results_table$Parameter == "rLEN Amplitude"]

# --- Data Preparation for Combined Plot ---
# Create data frames for each condition
LD_TIL4_vector_data <- data.frame(
  Condition = "LD",
  acrophase = LD_TIL4_acrophase,
  amplitude = LD_TIL4_amplitude
)

rLEN_TIL4_vector_data <- data.frame(
  Condition = "rLEN",
  acrophase = rLEN_TIL4_acrophase,
  amplitude = rLEN_TIL4_amplitude
)

# Combine the two data frames into a single one
combined_TIL4_vector_data <- rbind(LD_TIL4_vector_data, rLEN_TIL4_vector_data) %>%
  arrange(desc(amplitude))


# --- 1. Create the Main Plot ---
max_amplitude <- max(combined_TIL4_vector_data$amplitude, na.rm = TRUE)
...
main_plot_with_legend <- ggplot(combined_TIL4_vector_data, aes(color = Condition)) +
  ...
  labs(
    title = "Thoracic IL4 Group Acrophase Comparison (LD vs. rLEN)",
...
ggsave(
  filename = file.path(rayleigh_dir, "ThoracicIL4_Plot.pdf"),  
  plot = final_plot,
