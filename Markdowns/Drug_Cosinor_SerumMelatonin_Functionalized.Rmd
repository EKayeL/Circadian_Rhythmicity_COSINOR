---
title: "Drug Group Cosinor Serum Melatonin Functionalized"
...
### Reading, Analyzing and Plotting Serum Melatonin Administered Rats
```{r}
# Reading Serum Melatonin Administered Rats for LD
readxl::read_excel(path ="../Circadian Disruption ZT Data.xlsx",
                   sheet = "Serum Melatonin",
                    range ="N2:R11") -> Circadian_MelaSerum_LD

# Reading Serum Melatonin Administered Rats for rLEN
readxl::read_excel(path ="../Circadian Disruption ZT Data.xlsx",
                   sheet = "Serum Melatonin",
                    range ="U2:Y12") -> Circadian_MelaSerum_rLEN
```


#### Converting Data into Usable Format for Extracting VOIs and Plotting
``` {r}
# Data Conversion to get Individual Points for LD Serum Melatonin Rat Model
# X value is Zeitgeber time, Y value is Concentration in mg of Drug/Content


# Loads Initial Dataset for LD Serum Melatonin Rats and Removes Outliers
outlier_detection(Circadian_MelaSerum_LD) -> out_res_LDMS
filter_outliers(Circadian_MelaSerum_LD, 
                out_res_LDMS) ->  LD_Circadian_MelaSerum_Filter

# Performs Data Carpentry on Filtered LD MS Dataset and Removes 'NA's,
## filters out ZT of 24
LD_Circadian_MelaSerum_Filter %>%
  pivot_longer(cols = c(`0`,`6`,`12`, `18`, `24`), names_to = "ZT", 
               values_to = "Concentration(mg)") %>%
  drop_na(`Concentration(mg)`) %>%
  mutate(ZT = as.numeric(ZT)) %>%
  arrange(ZT) -> LD_Circadian_MelaSerum_Filter
LD_Circadian_MelaSerum_Filter %>%
  # Filter out ZT of 24
  filter(ZT != 24)  -> LD_Circadian_MelaSerum_data

# Creating Cosinor Model for LD MS Rat Plots
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = LD_Circadian_MelaSerum_Filter) -> cosMod_LD_MS
# Creating Cosinor Model for LD MS Rat Data Analysis
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = LD_Circadian_MelaSerum_data) -> cosMod_LD_MS_data
```

```{r}
# Data Conversion to get Individual Points for rLEN Serum Melatonin Rat Model
# X value is Zeitgeber time, Y value is Concentration in mg of Drug/Content

# Loads Initial Dataset for LD Serum Melatonin Rats and Removes Outliers
outlier_detection(Circadian_MelaSerum_rLEN) -> out_res_rLENMS
filter_outliers(Circadian_MelaSerum_rLEN, 
                out_res_rLENMS) ->  rLEN_Circadian_MelaSerum_Filter

# Performs Data Carpentry on Filtered LD MS Dataset and Removes 'NA's
rLEN_Circadian_MelaSerum_Filter %>%
  pivot_longer(cols = c(`0`,`6`,`12`, `18`, `24`), names_to = "ZT", 
               values_to = "Concentration(mg)") %>%
  drop_na(`Concentration(mg)`) %>%
  mutate(ZT = as.numeric(ZT)) %>%
  arrange(ZT) -> rLEN_Circadian_MelaSerum_Filter
rLEN_Circadian_MelaSerum_Filter %>%
  # Filter out ZT of 24
  filter(ZT != 24)  -> rLEN_Circadian_MelaSerum_data

# Creating Cosinor Model for LD MS Rat Plots
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = rLEN_Circadian_MelaSerum_Filter) -> cosMod_rLEN_MS
# Creating Cosinor Model for LD MS Rat Plot Data Analysis
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = rLEN_Circadian_MelaSerum_data) -> cosMod_rLEN_MS_data
```
...
# Plot
max(rLEN_Circadian_MelaSerum_Filter$`Concentration(mg)`, 
    LD_Circadian_MelaSerum_Filter$`Concentration(mg)`) -> MS_max
ceiling(MS_max / 5) * 5 -> y_peak
```
...
plot_cosinor(cosMod_LD_MS, LD_Circadian_MelaSerum_Filter, 
             "LD Serum Melatonin", "LD",
             x_breaks = seq(0, 24, by = 6)) -> mod_plot_LD_MS


# Creating Plot for rLEN CS Rat Model
plot_cosinor(cosMod_rLEN_MS, rLEN_Circadian_MelaSerum_Filter, 
             "rLEN Serum Melatonin", "rLEN",
             x_breaks = seq(0, 24, by = 6)) -> mod_plot_rLEN_MS
```


#### Performing Cosinor Analysis for Rat Datasets
```{r}
# Creating Table of Data VOIs for LD Serum Melatonin Rat Model
...
extract_cosinor(cosMod_LD_MS_data, LD_Circadian_MelaSerum_data,
                "LD") -> LD_extract_output
LD_MS_Results_table <- LD_extract_output$results_table
LD_MS_Results_grob <- LD_extract_output$results_grob


# Creating Table of Data VOIs for rLEN Serum Melatonin Rat Model
## Produces Table of Pertinent Values (Mesor, Amplitude, Acrophase, Bathyphase)
## and Converts Table into Graphical Object for Figure Creation
extract_cosinor(cosMod_rLEN_MS_data, rLEN_Circadian_MelaSerum_data,
                "rLEN") -> rLEN_extract_output
rLEN_MS_Results_table <- rLEN_extract_output$results_table
rLEN_MS_Results_grob <- rLEN_extract_output$results_grob
```
...
## Creating Table of Zero Amplitude VOIs for LD Serum Melatonin Rat Model

# Produces Table of Pertinent Values (df1, df2, f-value, p-value) and Converts 
# Table into Graphical Object for Figure Creation
zero_amp(cosMod_LD_MS_data, "LD") -> LD_zero_output
LD_MS_amp_test_Results <- LD_zero_output$results_table
LD_MS_amp_test_grob <- LD_zero_output$results_grob


# Produces Table of Pertinent Values (df1, df2, f-value, p-value) and Converts 
# Table into Graphical Object for Figure Creation
zero_amp(cosMod_rLEN_MS_data, "rLEN") -> rLEN_zero_output
rLEN_MS_amp_test_Results <- rLEN_zero_output$results_table
rLEN_MS_amp_test_grob <- rLEN_zero_output$results_grob
```
...
pdf(file.path(plot_dir, "combined_MS_plots.pdf"), width=15, height=6)
grid.arrange(mod_plot_LD_MS, mod_plot_rLEN_MS, ncol = 2)
dev.off()
```
...
tables <- list(LD_MS_Results_grob, LD_MS_amp_test_grob, 
               rLEN_MS_Results_grob, rLEN_MS_amp_test_grob, NULL, NULL)
title_text <- "Comparison of Melatonin Serum LD and rLEN Circadian Disruption Values"
output_file <- file.path(table_dir, "combined_MS_tables.pdf")
fig_width <- 7.5
fig_height <- 10
...
# --- Data Extraction ---
# Extract LD Serum Melatonin (MS) data
LD_MS_acrophase <- LD_MS_Results_table$Values[LD_MS_Results_table$Parameter == "LD Acrophase (Hrs)"]
LD_MS_amplitude <- LD_MS_Results_table$Values[LD_MS_Results_table$Parameter == "LD Amplitude"]

# Extract rLEN Serum Melatonin (MS) data
rLEN_MS_acrophase <- rLEN_MS_Results_table$Values[rLEN_MS_Results_table$Parameter == "rLEN Acrophase (Hrs)"]
rLEN_MS_amplitude <- rLEN_MS_Results_table$Values[rLEN_MS_Results_table$Parameter == "rLEN Amplitude"]

# --- Data Preparation for Combined Plot ---
# Create data frames for each condition
LD_MS_vector_data <- data.frame(
  Condition = "LD",
  acrophase = LD_MS_acrophase,
  amplitude = LD_MS_amplitude
)

rLEN_MS_vector_data <- data.frame(
  Condition = "rLEN",
  acrophase = rLEN_MS_acrophase,
  amplitude = rLEN_MS_amplitude
)

# Combine the two data frames into a single one
combined_MS_vector_data <- rbind(LD_MS_vector_data, rLEN_MS_vector_data) %>%
  arrange(desc(amplitude))


# --- 1. Create the Main Plot ---
max_amplitude <- max(combined_MS_vector_data$amplitude, na.rm = TRUE)
...
main_plot_with_legend <- ggplot(combined_MS_vector_data, aes(color = Condition)) +
  ...
  labs(
    title = "Serum Melatonin Group Acrophase Comparison (LD vs. rLEN)",
...
ggsave(
  filename = file.path(rayleigh_dir, "SerumMelatonin_Plot.pdf"),  
  plot = final_plot,
