---
title: "Drug Cosinor Serum Norepinephrine"
...
### Reading, Converting, Analyzing and Plotting Serum Norepinephrine 
### Administered Rats 
#### Reading in Serum Norepinephrine Administered Rat Data for 
#### LD and rLEN
```{r}
# Reading Serum Norepinephrine Administered Rats for LD
readxl::read_excel(path ="../Circadian Disruption ZT Data ABM NEW.xlsx",
                   sheet = "Serum Norepinephrine",
                    range ="N2:R11") -> Circadian_SerumNorep_LD

# Reading Serum Norepinephrine Administered Rats for rLEN
readxl::read_excel(path ="../Circadian Disruption ZT Data ABM NEW.xlsx",
                   sheet = "Serum Norepinephrine",
                    range ="U2:Y11") -> Circadian_SerumNorep_rLEN
```


#### Converting Data into Usable Format for Extracting VOIs and Plotting
```{r}
# Data Conversion to get Individual Points for LD Serum Norepinephrine 
# Rat Model
## X value is Zeitgeber time, Y value is Concentration in mg of Drug/Content

# Loads Initial Dataset for LD Serum Norepinephrine Rats and 
# Removes Outliers
outlier_detection(Circadian_SerumNorep_LD) -> out_res_LDSNE
filter_outliers(Circadian_SerumNorep_LD, 
                out_res_LDSNE) ->  LD_Circadian_SerumNorep_Filter

# Performs Data Carpentry on Filtered LD SNE Dataset and Removes 'NA's,
## filters out ZT of 24
LD_Circadian_SerumNorep_Filter %>%
  pivot_longer(cols = c(`0`,`6`,`12`, `18`, `24`), names_to = "ZT", 
               values_to = "Concentration(mg)") %>%
  drop_na(`Concentration(mg)`) %>%
  mutate(ZT = as.numeric(ZT)) %>%
  arrange(ZT) -> LD_Circadian_SerumNorep_Filter
LD_Circadian_SerumNorep_Filter %>%
  # Filter out ZT of 24
  filter(ZT != 24)  -> LD_Circadian_SerumNorep_data

# Creating Cosinor Model for LD SNE Rat Plots
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = LD_Circadian_SerumNorep_Filter) -> cosMod_LD_SNE
# Creating Cosinor Model for LD SNE Rat Data Analysis
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = LD_Circadian_SerumNorep_data) -> cosMod_LD_SNE_data
```

```{r}
# Data Conversion to get Individual Points for rLEN Serum Norepinephrine 
# Rat Model X value is Zeitgeber time, Y value is Concentration in 
# mg of Drug/Content

# Loads Initial Dataset for rLEN Serum Norepinephrine Rats and Removes Outliers
outlier_detection(Circadian_SerumNorep_rLEN) -> out_res_rLENSNE
filter_outliers(Circadian_SerumNorep_rLEN, 
                out_res_rLENSNE) ->  rLEN_Circadian_SerumNorep_Filter

# Performs Data Carpentry on Filtered LD SNE Dataset and Removes 'NA's
rLEN_Circadian_SerumNorep_Filter %>%
  pivot_longer(cols = c(`0`,`6`,`12`, `18`, `24`), names_to = "ZT", 
               values_to = "Concentration(mg)") %>%
  drop_na(`Concentration(mg)`) %>%
  mutate(ZT = as.numeric(ZT)) %>%
  arrange(ZT) -> rLEN_Circadian_SerumNorep_Filter
rLEN_Circadian_SerumNorep_Filter %>%
  # Filter out ZT of 24
  filter(ZT != 24)  -> rLEN_Circadian_SerumNorep_data

# Creating Cosinor Model for LD SNE Rat Plots
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = rLEN_Circadian_SerumNorep_Filter) -> cosMod_rLEN_SNE
# Creating Cosinor Model for LD SNE Rat Plot Data Analysis
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = rLEN_Circadian_SerumNorep_data) -> cosMod_rLEN_SNE_data
```
...
# Future Plot
max(rLEN_Circadian_SerumNorep_Filter$`Concentration(mg)`, 
    LD_Circadian_SerumNorep_Filter$`Concentration(mg)`) -> SNE_max
ceiling(SNE_max / 5) * 5 -> y_peak
```
...
# Creating Plot for LD SNE Rat Model
plot_cosinor(cosMod_LD_SNE, LD_Circadian_SerumNorep_Filter, 
             "LD Serum Norepinephrine", "LD",
             x_breaks = seq(0, 24, by = 6)) -> mod_plot_LD_SNE


# Creating Plot for rLEN SNE Rat Model
plot_cosinor(cosMod_rLEN_SNE, rLEN_Circadian_SerumNorep_Filter, 
             "rLEN Serum Norepinephrine", "rLEN",
             x_breaks = seq(0, 24, by = 6)) -> mod_plot_rLEN_SNE
```


#### Performing Cosinor Analysis for Rat Datasets
```{r}
# Creating Table of Data VOIs for LD Serum Norepinephrine Rat Model
...
extract_cosinor(cosMod_LD_SNE_data, LD_Circadian_SerumNorep_data,
                "LD") -> LD_extract_output
LD_SNE_Results_table <- LD_extract_output$results_table
LD_SNE_Results_grob <- LD_extract_output$results_grob


# Creating Table of Data VOIs for rLEN Serum Norepinephrine Rat Model
## Produces Table of Pertinent Values (Mesor, Amplitude, Acrophase, Bathyphase)
## and Converts Table into Graphical Object for Figure Creation
extract_cosinor(cosMod_rLEN_SNE_data, rLEN_Circadian_SerumNorep_data,
                "rLEN") -> rLEN_extract_output
rLEN_SNE_Results_table <- rLEN_extract_output$results_table
rLEN_SNE_Results_grob <- rLEN_extract_output$results_grob
```
...
## Creating Table of Zero Amplitude VOIs for LD Serum Norepinephrine Rat Model

# Produces Table of Pertinent Values (df1, df2, f-value, p-value) and Converts 
# Table into Graphical Object for Figure Creation
zero_amp(cosMod_LD_SNE_data, "LD") -> LD_zero_output
LD_SNE_amp_test_Results <- LD_zero_output$results_table
LD_SNE_amp_test_grob <- LD_zero_output$results_grob


# Produces Table of Pertinent Values (df1, df2, f-value, p-value) and Converts 
# Table into Graphical Object for Figure Creation
zero_amp(cosMod_rLEN_SNE_data, "rLEN") -> rLEN_zero_output
rLEN_SNE_amp_test_Results <- rLEN_zero_output$results_table
rLEN_SNE_amp_test_grob <- rLEN_zero_output$results_grob
```
...
pdf(file.path(plot_dir, "combined_SNE_plots.pdf"), width=15, height=6)
grid.arrange(mod_plot_LD_SNE, mod_plot_rLEN_SNE, ncol = 2)
dev.off()
```
...
tables <- list(LD_SNE_Results_grob, LD_SNE_amp_test_grob, 
               rLEN_SNE_Results_grob, rLEN_SNE_amp_test_grob, NULL, NULL)
title_text <- "Comparison of Serum Norepinephrine LD and rLEN Circadian Disruption Values"
output_file <- file.path(table_dir, "combined_SNE_tables.pdf")
fig_width <- 7.5
fig_height <- 10
...
# --- Data Extraction ---
# Extract LD Serum Norepinephrine (SNE) data
LD_SNE_acrophase <- LD_SNE_Results_table$Values[LD_SNE_Results_table$Parameter == "LD Acrophase (Hrs)"]
LD_SNE_amplitude <- LD_SNE_Results_table$Values[LD_SNE_Results_table$Parameter == "LD Amplitude"]

# Extract rLEN Serum Norepinephrine (SNE) data
rLEN_SNE_acrophase <- rLEN_SNE_Results_table$Values[rLEN_SNE_Results_table$Parameter == "rLEN Acrophase (Hrs)"]
rLEN_SNE_amplitude <- rLEN_SNE_Results_table$Values[rLEN_SNE_Results_table$Parameter == "rLEN Amplitude"]

# --- Data Preparation for Combined Plot ---
# Create data frames for each condition
LD_SNE_vector_data <- data.frame(
  Condition = "LD",
  acrophase = LD_SNE_acrophase,
  amplitude = LD_SNE_amplitude
)

rLEN_SNE_vector_data <- data.frame(
  Condition = "rLEN",
  acrophase = rLEN_SNE_acrophase,
  amplitude = rLEN_SNE_amplitude
)

# Combine the two data frames into a single one
combined_SNE_vector_data <- rbind(LD_SNE_vector_data, rLEN_SNE_vector_data) %>%
  arrange(desc(amplitude))


# --- 1. Create the Main Plot ---
max_amplitude <- max(combined_SNE_vector_data$amplitude, na.rm = TRUE)
...
main_plot_with_legend <- ggplot(combined_SNE_vector_data, aes(color = Condition)) +
  ...
  labs(
    title = "Serum Norepinephrine Group Acrophase Comparison (LD vs. rLEN)",
...
ggsave(
  filename = file.path(rayleigh_dir, "SerumNorepinephrine_Plot.pdf"),  
  plot = final_plot,
