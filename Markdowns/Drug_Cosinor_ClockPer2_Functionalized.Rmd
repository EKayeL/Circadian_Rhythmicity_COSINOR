---
title: "Drug Group Cosinor Clock Per2 Functionalized"
...
extract_cosinor_clock(cosMod_LD_CPer2_data, LD_Circadian_Clock_Per2_data,
...
## Produces Table of Pertinent Values (Mesor, Amplitude, Acrophase, Bathyphase)
## and Converts Table into Graphical Object for Figure Creation
extract_cosinor_clock(cosMod_rLEN_CPer2_data, rLEN_Circadian_Clock_Per2_data,
...
zero_amp(cosMod_LD_CPer2_data, "LD") -> LD_zero_output
...
# Produces Table of Pertinent Values (df1, df2, f-value, p-value) and Converts 
# Table into Graphical Object for Figure Creation
zero_amp(cosMod_rLEN_CPer2_data, "rLEN") -> rLEN_zero_output
...
pdf(file.path(plot_dir, "combined_CPer2_plots.pdf"), width=15, height=6)
...
title_text <- "Comparison of Clock Per2 LD and rLEN Fold Change Values"
output_file <- file.path(table_dir, "combined_CPer2_tables.pdf")
fig_width <- 7.5
fig_height <- 10
...
# --- Data Extraction ---
# Extract LD Clock Per2 (CPer2) data
LD_CPer2_acrophase <- LD_CPer2_Results_table$Values[LD_CPer2_Results_table$Parameter == "LD Acrophase (Hrs)"]
LD_CPer2_amplitude <- LD_CPer2_Results_table$Values[LD_CPer2_Results_table$Parameter == "LD Amplitude"]

# Extract rLEN Clock Per2 (CPer2) data
rLEN_CPer2_acrophase <- rLEN_CPer2_Results_table$Values[rLEN_CPer2_Results_table$Parameter == "rLEN Acrophase (Hrs)"]
rLEN_CPer2_amplitude <- rLEN_CPer2_Results_table$Values[rLEN_CPer2_Results_table$Parameter == "rLEN Amplitude"]

# --- Data Preparation for Combined Plot ---
# Create data frames for each condition
LD_CPer2_vector_data <- data.frame(
  Condition = "LD",
  acrophase = LD_CPer2_acrophase,
  amplitude = LD_CPer2_amplitude
)

rLEN_CPer2_vector_data <- data.frame(
  Condition = "rLEN",
  acrophase = rLEN_CPer2_acrophase,
  amplitude = rLEN_CPer2_amplitude
)

# Combine the two data frames into a single one
combined_CPer2_vector_data <- rbind(LD_CPer2_vector_data, rLEN_CPer2_vector_data) %>%
  arrange(desc(amplitude))


# --- 1. Create the Main Plot ---
max_amplitude <- max(combined_CPer2_vector_data$amplitude, na.rm = TRUE)
...
main_plot_with_legend <- ggplot(combined_CPer2_vector_data, aes(color = Condition)) +
  ...
  labs(
    title = "Clock Per2 Group Acrophase Comparison (LD vs. rLEN)",
...
ggsave(
  filename = file.path(rayleigh_dir, "ClockPer2_Plot.pdf"),  
  plot = final_plot,
