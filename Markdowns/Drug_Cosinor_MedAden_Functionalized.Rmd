---
title: "Drug Group Cosinor Medullary Adenosine Functionalized"
...
### Reading, Converting, Analyzing and Plotting Medullary Adenosine 
### Administered Rats 
#### Reading in Medullary Adenosine Administered Rat Data for LD and rLEN
```{r}
# Reading Medullary Adenosine Administered Rats for LD
readxl::read_excel(path ="../Circadian Disruption ZT Data.xlsx",
                   sheet = "Medullary Adenosine",
                    range ="N2:R8") -> Circadian_MedAden_LD

# Reading Medullary Adenosine Administered Rats for rLEN
readxl::read_excel(path ="../Circadian Disruption ZT Data.xlsx",
                   sheet = "Medullary Adenosine",
                    range ="U2:Y7") -> Circadian_MedAden_rLEN
```


#### Converting Data into Usable Format for Extracting VOIs and Plotting
```{r}
# Data Conversion to get Individual Points for LD Medullary Adenosine Rat Model
## X value is Zeitgeber time, Y value is Concentration in mg of Drug/Content

# Loads Initial Dataset for LD Medullary Adenosine Rats and Removes Outliers
outlier_detection(Circadian_MedAden_LD) -> out_res_LDMA
filter_outliers(Circadian_MedAden_LD, 
                out_res_LDMA) ->  LD_Circadian_MedAden_Filter

# Performs Data Carpentry on Filtered LD MS Dataset and Removes 'NA's,
## filters out ZT of 24
LD_Circadian_MedAden_Filter %>%
  pivot_longer(cols = c(`0`,`6`,`12`, `18`, `24`), names_to = "ZT", 
               values_to = "Concentration(mg)") %>%
  drop_na(`Concentration(mg)`) %>%
  mutate(ZT = as.numeric(ZT)) %>%
  arrange(ZT) -> LD_Circadian_MedAden_Filter
LD_Circadian_MedAden_Filter %>%
  # Filter out ZT of 24
  filter(ZT != 24)  -> LD_Circadian_MedAden_data

# Creating Cosinor Model for LD MS Rat Plots
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = LD_Circadian_MedAden_Filter) -> cosMod_LD_MA
# Creating Cosinor Model for LD MS Rat Data Analysis
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = LD_Circadian_MedAden_data) -> cosMod_LD_MA_data
```

```{r}
# Data Conversion to get Individual Points for rLEN Medullary Adenosine 
# Rat Model 
## X value is Zeitgeber time, Y value is Concentration in mg of Drug/Content

# Loads Initial Dataset for rLEN Medullary Adenosine Rats and Removes Outliers
outlier_detection(Circadian_MedAden_rLEN) -> out_res_rLENMA
filter_outliers(Circadian_MedAden_rLEN, 
                out_res_rLENMA) ->  rLEN_Circadian_MedAden_Filter

# Performs Data Carpentry on Filtered LD MS Dataset and Removes 'NA's
rLEN_Circadian_MedAden_Filter %>%
  pivot_longer(cols = c(`0`,`6`,`12`, `18`, `24`), names_to = "ZT", 
               values_to = "Concentration(mg)") %>%
  drop_na(`Concentration(mg)`) %>%
  mutate(ZT = as.numeric(ZT)) %>%
  arrange(ZT) -> rLEN_Circadian_MedAden_Filter
rLEN_Circadian_MedAden_Filter %>%
  # Filter out ZT of 24
  filter(ZT != 24)  -> rLEN_Circadian_MedAden_data

# Creating Cosinor Model for LD MS Rat Plots
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = rLEN_Circadian_MedAden_Filter) -> cosMod_rLEN_MA
# Creating Cosinor Model for LD MS Rat Plot Data Analysis
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = rLEN_Circadian_MedAden_data) -> cosMod_rLEN_MA_data
```
...
# Future Plot
max(rLEN_Circadian_MedAden_Filter$`Concentration(mg)`, 
    LD_Circadian_MedAden_Filter$`Concentration(mg)`) -> MA_max
ceiling(MA_max / 5) * 5 -> y_peak
```
...
plot_cosinor(cosMod_LD_MA, LD_Circadian_MedAden_Filter, 
             "LD Medullary Adenosine", "LD",
             x_breaks = seq(0, 24, by = 6)) -> mod_plot_LD_MA


# Creating Plot for rLEN CS Rat Model
plot_cosinor(cosMod_rLEN_MA, rLEN_Circadian_MedAden_Filter, 
             "rLEN Medullary Adenosine", "rLEN",
             x_breaks = seq(0, 24, by = 6)) -> mod_plot_rLEN_MA
```


#### Performing Cosinor Analysis for Rat Datasets
```{r}
# Creating Table of Data VOIs for LD Medullary Adenosine Rat Model
...
extract_cosinor(cosMod_LD_MA_data, LD_Circadian_MedAden_data,
                "LD") -> LD_extract_output
LD_MA_Results_table <- LD_extract_output$results_table
LD_MA_Results_grob <- LD_extract_output$results_grob


# Creating Table of Data VOIs for rLEN Medullary Adenosine Rat Model
## Produces Table of Pertinent Values (Mesor, Amplitude, Acrophase, Bathyphase)
## and Converts Table into Graphical Object for Figure Creation
extract_cosinor(cosMod_rLEN_MA_data, rLEN_Circadian_MedAden_data,
                "rLEN") -> rLEN_extract_output
rLEN_MA_Results_table <- rLEN_extract_output$results_table
rLEN_MA_Results_grob <- rLEN_extract_output$results_grob
```
...
## Creating Table of Zero Amplitude VOIs for LD Medullary Adenosine Rat Model

# Produces Table of Pertinent Values (df1, df2, f-value, p-value) and Converts 
# Table into Graphical Object for Figure Creation
zero_amp(cosMod_LD_MA_data, "LD") -> LD_zero_output
LD_MA_amp_test_Results <- LD_zero_output$results_table
LD_MA_amp_test_grob <- LD_zero_output$results_grob


# Produces Table of Pertinent Values (df1, df2, f-value, p-value) and Converts 
# Table into Graphical Object for Figure Creation
zero_amp(cosMod_rLEN_MA_data, "rLEN") -> rLEN_zero_output
rLEN_MA_amp_test_Results <- rLEN_zero_output$results_table
rLEN_MA_amp_test_grob <- rLEN_zero_output$results_grob
```
...
pdf(file.path(plot_dir, "combined_MA_plots.pdf"), width=15, height=6)
grid.arrange(mod_plot_LD_MA, mod_plot_rLEN_MA, ncol = 2)
dev.off()
```
...
tables <- list(LD_MA_Results_grob, LD_MA_amp_test_grob, 
               rLEN_MA_Results_grob, rLEN_MA_amp_test_grob, NULL, NULL)
title_text <- "Comparison of Medullary Adenosine LD and rLEN Circadian Disruption Values"
output_file <- file.path(table_dir, "combined_MA_tables.pdf")
fig_width <- 7.5
fig_height <- 10
...
# --- Data Extraction ---
# Extract LD Medullary Adenosine (MA) data
LD_MA_acrophase <- LD_MA_Results_table$Values[LD_MA_Results_table$Parameter == "LD Acrophase (Hrs)"]
LD_MA_amplitude <- LD_MA_Results_table$Values[LD_MA_Results_table$Parameter == "LD Amplitude"]

# Extract rLEN Medullary Adenosine (MA) data
rLEN_MA_acrophase <- rLEN_MA_Results_table$Values[rLEN_MA_Results_table$Parameter == "rLEN Acrophase (Hrs)"]
rLEN_MA_amplitude <- rLEN_MA_Results_table$Values[rLEN_MA_Results_table$Parameter == "rLEN Amplitude"]

# --- Data Preparation for Combined Plot ---
# Create data frames for each condition
LD_MA_vector_data <- data.frame(
  Condition = "LD",
  acrophase = LD_MA_acrophase,
  amplitude = LD_MA_amplitude
)

rLEN_MA_vector_data <- data.frame(
  Condition = "rLEN",
  acrophase = rLEN_MA_acrophase,
  amplitude = rLEN_MA_amplitude
)

# Combine the two data frames into a single one
combined_MA_vector_data <- rbind(LD_MA_vector_data, rLEN_MA_vector_data) %>%
  arrange(desc(amplitude))


# --- 1. Create the Main Plot ---
max_amplitude <- max(combined_MA_vector_data$amplitude, na.rm = TRUE)
...
main_plot_with_legend <- ggplot(combined_MA_vector_data, aes(color = Condition)) +
  ...
  labs(
    title = "Medullary Adenosine Group Acrophase Comparison (LD vs. rLEN)",
...
ggsave(
  filename = file.path(rayleigh_dir, "MedullaryAdenosine_Plot.pdf"),  
  plot = final_plot,
