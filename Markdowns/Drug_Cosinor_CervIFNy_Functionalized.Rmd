...
# --- Data Extraction ---
# Extract LD Cervical IFNy (CIFNy) data
LD_CIFNy_acrophase <- LD_CIFNy_Results_table$Values[LD_CIFNy_Results_table$Parameter == "LD Acrophase (Hrs)"]
LD_CIFNy_amplitude <- LD_CIFNy_Results_table$Values[LD_CIFNy_Results_table$Parameter == "LD Amplitude"]

# Extract rLEN Cervical IFNy (CIFNy) data
rLEN_CIFNy_acrophase <- rLEN_CIFNy_Results_table$Values[rLEN_CIFNy_Results_table$Parameter == "rLEN Acrophase (Hrs)"]
rLEN_CIFNy_amplitude <- rLEN_CIFNy_Results_table$Values[rLEN_CIFNy_Results_table$Parameter == "rLEN Amplitude"]

# --- Data Preparation for Combined Plot ---
# Create data frames for each condition
LD_CIFNy_vector_data <- data.frame(
  Condition = "LD",
  acrophase = LD_CIFNy_acrophase,
  amplitude = LD_CIFNy_amplitude
)

rLEN_CIFNy_vector_data <- data.frame(
  Condition = "rLEN",
  acrophase = rLEN_CIFNy_acrophase,
  amplitude = rLEN_CIFNy_amplitude
)

# Combine the two data frames into a single one
combined_CIFNy_vector_data <- rbind(LD_CIFNy_vector_data, rLEN_CIFNy_vector_data) %>%
  arrange(desc(amplitude))


# --- 1. Create the Main Plot ---
max_amplitude <- max(combined_CIFNy_vector_data$amplitude, na.rm = TRUE)
...
main_plot_with_legend <- ggplot(combined_CIFNy_vector_data, aes(color = Condition)) +
  ...
  labs(
    title = "Cervical IFNy Group Acrophase Comparison (LD vs. rLEN)",
...
ggsave(
  filename = file.path(rayleigh_dir, "CervicalIFNy_Plot.pdf"),  
  plot = final_plot,
