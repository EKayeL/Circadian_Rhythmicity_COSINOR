---
title: "Drug Group Cosinor Medullary IL-2 Functionalized"
...
### Reading, Converting, Analyzing and Plotting Medullary IL-2 
### Administered Rats 
#### Reading in Medullary IL-2 Administered Rat Data for 
#### LD and rLEN
```{r}
# Reading Medullary IL-2 Administered Rats for LD
readxl::read_excel(path ="../Summary Multiplex Data.xlsx",
                   sheet = "Medulla",
                    range ="N20:R26") -> LD_Circadian_Med_IL2_Filter

# Reading Medullary IL-2 Administered Rats for rLEN
readxl::read_excel(path ="../Summary Multiplex Data.xlsx",
                   sheet = "Medulla",
                    range ="U20:Y26") -> rLEN_Circadian_Med_IL2_Filter
```


#### Converting Data into Usable Format for Extracting VOIs and Plotting
```{r}
# Data Conversion to get Individual Points for LD Medullary IL-2 
# Rat Model
## X value is Zeitgeber time, Y value is Concentration in mg of Drug/Content

# Loads Initial Dataset for LD Medullary IL-2 Rats and 
# Removes Outliers
#outlier_detection(Circadian_Med_IL2_LD) -> out_res_LDMIL2
#filter_outliers(Circadian_Med_IL2_LD, 
#                out_res_LDMIL2) ->  LD_Circadian_Med_IL2_Filter

# Performs Data Carpentry on Filtered LD IL2 Dataset and Removes 'NA's,
## filters out ZT of 24
LD_Circadian_Med_IL2_Filter %>%
  pivot_longer(cols = c(`0`,`6`,`12`, `18`, `24`), names_to = "ZT", 
               values_to = "Concentration(mg)") %>%
  drop_na(`Concentration(mg)`) %>%
  mutate(ZT = as.numeric(ZT)) %>%
  arrange(ZT) -> LD_Circadian_Med_IL2_Filter
LD_Circadian_Med_IL2_Filter %>%
  # Filter out ZT of 24
  filter(ZT != 24)  -> LD_Circadian_Med_IL2_data

# Creating Cosinor Model for LD IL2 Rat Plots
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = LD_Circadian_Med_IL2_Filter) -> cosMod_LD_MIL2
# Creating Cosinor Model for LD IL2 Rat Data Analysis
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = LD_Circadian_Med_IL2_data) -> cosMod_LD_MIL2_data
```

```{r}
# Data Conversion to get Individual Points for rLEN Medullary IL-2 
# Rat Model X value is Zeitgeber time, Y value is Concentration in 
# mg of Drug/Content

# Loads Initial Dataset for rLEN Medullary IL-2 Rats and Removes Outliers
#outlier_detection(Circadian_Med_IL2_rLEN) -> out_res_rLENMIL2
#filter_outliers(Circadian_Med_IL2_rLEN, 
#                out_res_rLENMIL2) ->  rLEN_Circadian_Med_IL2_Filter

# Performs Data Carpentry on Filtered LD IL2 Dataset and Removes 'NA's
rLEN_Circadian_Med_IL2_Filter %>%
  pivot_longer(cols = c(`0`,`6`,`12`, `18`, `24`), names_to = "ZT", 
               values_to = "Concentration(mg)") %>%
  drop_na(`Concentration(mg)`) %>%
  mutate(ZT = as.numeric(ZT)) %>%
  arrange(ZT) -> rLEN_Circadian_Med_IL2_Filter
rLEN_Circadian_Med_IL2_Filter %>%
  # Filter out ZT of 24
  filter(ZT != 24)  -> rLEN_Circadian_Med_IL2_data

# Creating Cosinor Model for LD IL2 Rat Plots
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = rLEN_Circadian_Med_IL2_Filter) -> cosMod_rLEN_MIL2
# Creating Cosinor Model for LD IL2 Rat Plot Data Analysis
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = rLEN_Circadian_Med_IL2_data) -> cosMod_rLEN_MIL2_data
```
...
# Future Plot
max(rLEN_Circadian_Med_IL2_Filter$`Concentration(mg)`, 
    LD_Circadian_Med_IL2_Filter$`Concentration(mg)`) -> MIL2_max
ceiling(MIL2_max / 5) * 5 -> y_peak
```
...
plot_cosinor(cosMod_LD_MIL2, LD_Circadian_Med_IL2_Filter, 
             "LD Medullary IL-2", "LD",
             x_breaks = seq(0, 24, by = 6)) -> mod_plot_LD_MIL2


# Creating Plot for rLEN IL2 Rat Model
plot_cosinor(cosMod_rLEN_MIL2, rLEN_Circadian_Med_IL2_Filter, 
             "rLEN Medullary IL-2", "rLEN",
             x_breaks = seq(0, 24, by = 6)) -> mod_plot_rLEN_MIL2
```


#### Performing Cosinor Analysis for Rat Datasets
```{r}
# Creating Table of Data VOIs for LD Medullary IL-2 Rat Model
...
extract_cosinor(cosMod_LD_MIL2_data, LD_Circadian_Med_IL2_data,
                "LD") -> LD_extract_output
LD_MIL2_Results_table <- LD_extract_output$results_table
LD_MIL2_Results_grob <- LD_extract_output$results_grob


# Creating Table of Data VOIs for rLEN Medullary IL-2 Rat Model
## Produces Table of Pertinent Values (Mesor, Amplitude, Acrophase, Bathyphase)
## and Converts Table into Graphical Object for Figure Creation
extract_cosinor(cosMod_rLEN_MIL2_data, rLEN_Circadian_Med_IL2_data,
                "rLEN") -> rLEN_extract_output
rLEN_MIL2_Results_table <- rLEN_extract_output$results_table
rLEN_MIL2_Results_grob <- rLEN_extract_output$results_grob
```
...
## Creating Table of Zero Amplitude VOIs for LD Medullary IL-2 Rat Model

# Produces Table of Pertinent Values (df1, df2, f-value, p-value) and Converts 
# Table into Graphical Object for Figure Creation
zero_amp(cosMod_LD_MIL2_data, "LD") -> LD_zero_output
LD_MIL2_amp_test_Results <- LD_zero_output$results_table
LD_MIL2_amp_test_grob <- LD_zero_output$results_grob


# Produces Table of Pertinent Values (df1, df2, f-value, p-value) and Converts 
# Table into Graphical Object for Figure Creation
zero_amp(cosMod_rLEN_MIL2_data, "rLEN") -> rLEN_zero_output
rLEN_MIL2_amp_test_Results <- rLEN_zero_output$results_table
rLEN_MIL2_amp_test_grob <- rLEN_zero_output$results_grob
```
...
pdf(file.path(plot_dir, "combined_MIL2_plots.pdf"), width=15, height=6)
grid.arrange(mod_plot_LD_MIL2, mod_plot_rLEN_MIL2, ncol = 2)
dev.off()
```
...
tables <- list(LD_MIL2_Results_grob, LD_MIL2_amp_test_grob, 
               rLEN_MIL2_Results_grob, rLEN_MIL2_amp_test_grob, NULL, NULL)
title_text <- "Comparison of Medullary IL-2 LD and rLEN Circadian Disruption Values"
output_file <- file.path(table_dir, "combined_MIL2_tables.pdf")
fig_width <- 7.5
fig_height <- 10
...
# --- Data Extraction ---
# Extract LD Medullary IL2 (MIL2) data
LD_MIL2_acrophase <- LD_MIL2_Results_table$Values[LD_MIL2_Results_table$Parameter == "LD Acrophase (Hrs)"]
LD_MIL2_amplitude <- LD_MIL2_Results_table$Values[LD_MIL2_Results_table$Parameter == "LD Amplitude"]

# Extract rLEN Medullary IL2 (MIL2) data
rLEN_MIL2_acrophase <- rLEN_MIL2_Results_table$Values[rLEN_MIL2_Results_table$Parameter == "rLEN Acrophase (Hrs)"]
rLEN_MIL2_amplitude <- rLEN_MIL2_Results_table$Values[rLEN_MIL2_Results_table$Parameter == "rLEN Amplitude"]

# --- Data Preparation for Combined Plot ---
# Create data frames for each condition
LD_MIL2_vector_data <- data.frame(
  Condition = "LD",
  acrophase = LD_MIL2_acrophase,
  amplitude = LD_MIL2_amplitude
)

rLEN_MIL2_vector_data <- data.frame(
  Condition = "rLEN",
  acrophase = rLEN_MIL2_acrophase,
  amplitude = rLEN_MIL2_amplitude
)

# Combine the two data frames into a single one
combined_MIL2_vector_data <- rbind(LD_MIL2_vector_data, rLEN_MIL2_vector_data) %>%
  arrange(desc(amplitude))


# --- 1. Create the Main Plot ---
max_amplitude <- max(combined_MIL2_vector_data$amplitude, na.rm = TRUE)
...
main_plot_with_legend <- ggplot(combined_MIL2_vector_data, aes(color = Condition)) +
  ...
  labs(
    title = "Medullary IL2 Group Acrophase Comparison (LD vs. rLEN)",
...
ggsave(
  filename = file.path(rayleigh_dir, "MedullaryIL2_Plot.pdf"),  
  plot = final_plot,
