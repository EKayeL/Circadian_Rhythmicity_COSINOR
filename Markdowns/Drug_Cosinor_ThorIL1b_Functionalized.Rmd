---
title: "Drug Group Cosinor Thoracic IL-1b Functionalized"
...
### Reading, Converting, Analyzing and Plotting Thoracic IL-1b 
### Administered Rats 
#### Reading in Thoracic IL-1b Administered Rat Data for 
#### LD and rLEN
```{r}
# Reading Thoracic IL-1b Administered Rats for LD
readxl::read_excel(path ="../Summary Multiplex Data.xlsx",
                   sheet = "Thoracic",
                    range ="N11:R17") -> LD_Circadian_Thor_IL1b_Filter

# Reading Thoracic IL-1b Administered Rats for rLEN
readxl::read_excel(path ="../Summary Multiplex Data.xlsx",
                   sheet = "Thoracic",
                    range ="U11:Y17") -> rLEN_Circadian_Thor_IL1b_Filter
```


#### Converting Data into Usable Format for Extracting VOIs and Plotting
```{r}
# Data Conversion to get Individual Points for LD Thoracic IL-1b 
# Rat Model
## X value is Zeitgeber time, Y value is Concentration in mg of Drug/Content

# Loads Initial Dataset for LD Thoracic IL-1b Rats and 
# Removes Outliers
#outlier_detection(Circadian_Thor_IL1b_LD) -> out_res_LDTIL1b
#filter_outliers(Circadian_Thor_IL1b_LD, 
#                out_res_LDTIL1b) ->  LD_Circadian_Thor_IL1b_Filter

# Performs Data Carpentry on Filtered LD IL1b Dataset and Removes 'NA's,
## filters out ZT of 24
LD_Circadian_Thor_IL1b_Filter %>%
  pivot_longer(cols = c(`0`,`6`,`12`, `18`, `24`), names_to = "ZT", 
               values_to = "Concentration(mg)") %>%
  drop_na(`Concentration(mg)`) %>%
  mutate(ZT = as.numeric(ZT)) %>%
  arrange(ZT) -> LD_Circadian_Thor_IL1b_Filter
LD_Circadian_Thor_IL1b_Filter %>%
  # Filter out ZT of 24
  filter(ZT != 24)  -> LD_Circadian_Thor_IL1b_data

# Creating Cosinor Model for LD IL1b Rat Plots
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = LD_Circadian_Thor_IL1b_Filter) -> cosMod_LD_TIL1b
# Creating Cosinor Model for LD IL1b Rat Data Analysis
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = LD_Circadian_Thor_IL1b_data) -> cosMod_LD_TIL1b_data
```

```{r}
# Data Conversion to get Individual Points for rLEN Thoracic IL-1b 
# Rat Model X value is Zeitgeber time, Y value is Concentration in 
# mg of Drug/Content

# Loads Initial Dataset for rLEN Thoracic IL-1b Rats and Removes Outliers
#outlier_detection(Circadian_Thor_IL1b_rLEN) -> out_res_rLENTIL1b
#filter_outliers(Circadian_Thor_IL1b_rLEN, 
#                out_res_rLENTIL1b) ->  rLEN_Circadian_Thor_IL1b_Filter

# Performs Data Carpentry on Filtered LD IL1b Dataset and Removes 'NA's
rLEN_Circadian_Thor_IL1b_Filter %>%
  pivot_longer(cols = c(`0`,`6`,`12`, `18`, `24`), names_to = "ZT", 
               values_to = "Concentration(mg)") %>%
  drop_na(`Concentration(mg)`) %>%
  mutate(ZT = as.numeric(ZT)) %>%
  arrange(ZT) -> rLEN_Circadian_Thor_IL1b_Filter
rLEN_Circadian_Thor_IL1b_Filter %>%
  # Filter out ZT of 24
  filter(ZT != 24)  -> rLEN_Circadian_Thor_IL1b_data

# Creating Cosinor Model for LD IL1b Rat Plots
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = rLEN_Circadian_Thor_IL1b_Filter) -> cosMod_rLEN_TIL1b
# Creating Cosinor Model for LD IL1b Rat Plot Data Analysis
cosinor.lm(`Concentration(mg)` ~ time(ZT), period = 24,
           data = rLEN_Circadian_Thor_IL1b_data) -> cosMod_rLEN_TIL1b_data
```
...
# Future Plot
max(rLEN_Circadian_Thor_IL1b_Filter$`Concentration(mg)`, 
    LD_Circadian_Thor_IL1b_Filter$`Concentration(mg)`) -> TIL1b_max
ceiling(TIL1b_max / 5) * 5 -> y_peak
```
...
# Creating Plot for LD IL1b Rat Model
plot_cosinor(cosMod_LD_TIL1b, LD_Circadian_Thor_IL1b_Filter, 
             "LD Thoracic IL-1b", "LD",
             x_breaks = seq(0, 24, by = 6)) -> mod_plot_LD_TIL1b


# Creating Plot for rLEN IL1b Rat Model
plot_cosinor(cosMod_rLEN_TIL1b, rLEN_Circadian_Thor_IL1b_Filter, 
             "rLEN Thoracic IL-1b", "rLEN",
             x_breaks = seq(0, 24, by = 6)) -> mod_plot_rLEN_TIL1b
```


#### Performing Cosinor Analysis for Rat Datasets
```{r}
# Creating Table of Data VOIs for LD Thoracic IL-1b Rat Model
...
extract_cosinor(cosMod_LD_TIL1b_data, LD_Circadian_Thor_IL1b_data,
                "LD") -> LD_extract_output
LD_TIL1b_Results_table <- LD_extract_output$results_table
LD_TIL1b_Results_grob <- LD_extract_output$results_grob


# Creating Table of Data VOIs for rLEN Thoracic IL-1b Rat Model
## Produces Table of Pertinent Values (Mesor, Amplitude, Acrophase, Bathyphase)
## and Converts Table into Graphical Object for Figure Creation
extract_cosinor(cosMod_rLEN_TIL1b_data, rLEN_Circadian_Thor_IL1b_data,
                "rLEN") -> rLEN_extract_output
rLEN_TIL1b_Results_table <- rLEN_extract_output$results_table
rLEN_TIL1b_Results_grob <- rLEN_extract_output$results_grob
```
...
## Creating Table of Zero Amplitude VOIs for LD Thoracic IL-1b Rat Model

# Produces Table of Pertinent Values (df1, df2, f-value, p-value) and Converts 
# Table into Graphical Object for Figure Creation
zero_amp(cosMod_LD_TIL1b_data, "LD") -> LD_zero_output
LD_TIL1b_amp_test_Results <- LD_zero_output$results_table
LD_TIL1b_amp_test_grob <- LD_zero_output$results_grob


# Produces Table of Pertinent Values (df1, df2, f-value, p-value) and Converts 
# Table into Graphical Object for Figure Creation
zero_amp(cosMod_rLEN_TIL1b_data, "rLEN") -> rLEN_zero_output
rLEN_TIL1b_amp_test_Results <- rLEN_zero_output$results_table
rLEN_TIL1b_amp_test_grob <- rLEN_zero_output$results_grob
```
...
pdf(file.path(plot_dir, "combined_TIL1b_plots.pdf"), width=15, height=6)
grid.arrange(mod_plot_LD_TIL1b, mod_plot_rLEN_TIL1b, ncol = 2)
dev.off()
```
...
tables <- list(LD_TIL1b_Results_grob, LD_TIL1b_amp_test_grob, 
               rLEN_TIL1b_Results_grob, rLEN_TIL1b_amp_test_grob, NULL, NULL)
title_text <- "Comparison of Thoracic IL-1b LD and rLEN Circadian Disruption Values"
output_file <- file.path(table_dir, "combined_TIL1b_tables.pdf")
fig_width <- 7.5
fig_height <- 10
...
# --- Data Extraction ---
# Extract LD Thoracic IL1b (TIL1b) data
LD_TIL1b_acrophase <- LD_TIL1b_Results_table$Values[LD_TIL1b_Results_table$Parameter == "LD Acrophase (Hrs)"]
LD_TIL1b_amplitude <- LD_TIL1b_Results_table$Values[LD_TIL1b_Results_table$Parameter == "LD Amplitude"]

# Extract rLEN Thoracic IL1b (TIL1b) data
rLEN_TIL1b_acrophase <- rLEN_TIL1b_Results_table$Values[rLEN_TIL1b_Results_table$Parameter == "rLEN Acrophase (Hrs)"]
rLEN_TIL1b_amplitude <- rLEN_TIL1b_Results_table$Values[rLEN_TIL1b_Results_table$Parameter == "rLEN Amplitude"]

# --- Data Preparation for Combined Plot ---
# Create data frames for each condition
LD_TIL1b_vector_data <- data.frame(
  Condition = "LD",
  acrophase = LD_TIL1b_acrophase,
  amplitude = LD_TIL1b_amplitude
)

rLEN_TIL1b_vector_data <- data.frame(
  Condition = "rLEN",
  acrophase = rLEN_TIL1b_acrophase,
  amplitude = rLEN_TIL1b_amplitude
)

# Combine the two data frames into a single one
combined_TIL1b_vector_data <- rbind(LD_TIL1b_vector_data, rLEN_TIL1b_vector_data) %>%
  arrange(desc(amplitude))


# --- 1. Create the Main Plot ---
max_amplitude <- max(combined_TIL1b_vector_data$amplitude, na.rm = TRUE)
...
main_plot_with_legend <- ggplot(combined_TIL1b_vector_data, aes(color = Condition)) +
  ...
  labs(
    title = "Thoracic IL1b Group Acrophase Comparison (LD vs. rLEN)",
...
ggsave(
  filename = file.path(rayleigh_dir, "ThoracicIL1b_Plot.pdf"),  
  plot = final_plot,
