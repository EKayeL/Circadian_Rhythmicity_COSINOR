...
#' @param data The dataset you wish to analyze. It must contain 'ZT' and 'Concentration(mg)' columns.
...
extract_cosinor <- function(cosMod, data, file_suffix) {
  ...
  if (exists(peak_ZT_name, envir = .GlobalEnv)) {
    ...
    if (abs(acrophase_hrs - peak_ZT) > 3) {
      ...
      # Ensure 'ZT' and 'Concentration(mg)' columns exist
      if (!("ZT" %in% colnames(data))) {
        stop("The data must contain a 'ZT' column representing time.")
      }
      if (!("Concentration(mg)" %in% colnames(data))) {
        stop("The data must contain a 'Concentration(mg)' column.")
      }
      
      # Extract the necessary columns
      ZT <- data$ZT
      Concentration_mg <- data$`Concentration(mg)`
      ...
      start_params_fixed <- list(
        amp = (max(Concentration_mg) - min(Concentration_mg)) / 2,
        phase = 0,  # Initial guess for phase
        offset = mean(Concentration_mg)
      )
      
      # Fit the model using nlsLM from the minpack.lm package
      fit_nlsLM <- nlsLM(
        formula = Concentration_mg ~ sinusoidal_model_fixed(ZT, amp, phase, offset),
        data = data.frame(ZT = ZT, Concentration_mg = Concentration_mg),
